--[[
 ...
 _____ _ _ ___________ ___________ 
/ ___ | \ | | _ _ | ___ \ ___ | ___ \
\ `-. | \ | | | | | | _ / / | __ | | _ / /
 `-. \. `| | | | __ / | __ || /
/ \ __ / / | \ | _ | | _ | | | | ___ | | \ \
...
local files_SNIPER = database:smembers("files"..bot_id) 
print(Green.."\nFiles Now Started : \n "..reset)
for i,v in pairs(files_SNIPER) do
print(Blue..i..red..' - \27[10;33m'..v..',\27[m')  end
print(Green.."\nThes all Files.\n\n\n"..reset)
io.popen("mkdir files_SNIPER")
...
local zain_add_sudo = redis:get('SNIPER:'..bot_id..'sudoo'..msg.sender_user_id_..'')
...
local var = false 
local admin = database:sismember('SNIPER:'..bot_id..'admins:', user_id)
if admin then var = true end
for k,v in pairs(sudo_users) do
if user_id == v then var = true end
end
local zain_add_sudo = redis:get('SNIPER:'..bot_id..'sudoo'..user_id..'')
...
local var = false
local creatorbasic = database:sismember('SNIPER:'..bot_id..'creatorbasic:'..chat_id, user_id) 
local creator = database:sismember('SNIPER:'..bot_id..'creator:'..chat_id, user_id) 
local admin = database:sismember('SNIPER:'..bot_id..'admins:', user_id)
...
chat_id = msg.chat_id_
local var = false
local mod = database:sismember('SNIPER:'..bot_id..'mods:'..chat_id, user_id)  
local admin = database:sismember('SNIPER:'..bot_id..'admins:', user_id)  
local owner = database:sismember('SNIPER:'..bot_id..'owners:'..chat_id, user_id)
local creator = database:sismember('SNIPER:'..bot_id..'creator:'..chat_id, user_id)  
local creatorbasic = database:sismember('SNIPER:'..bot_id..'creatorbasic:'..chat_id, user_id)  
local vip = database:sismember('SNIPER:'..bot_id..'vipgp:'..chat_id, user_id)
...
local var = false
local banned = database:sismember('SNIPER:'..bot_id..'banned:'..chat_id, user_id)
...
local var = false
local banned = database:sismember('SNIPER:'..bot_id..'gbanned:', user_id)
if banned then var = true end
return var
end
--         »»                 is_muted                         ««              --
...
local var = false
local banned = database:sismember('SNIPER:'..bot_id..'muted:'..chat_id, user_id)
if banned then var = true end
return var
end
--         »»                 is_gmuted                         ««              --
...
local var = false 
local banned = database:sismember('SNIPER:'..bot_id..'gmuted:', user_id)
if banned then var = true end
return var
end
...
k2342 = io.open("SNIPER.lua")
--         »»                 check_filter_words                         ««              --
local function check_filter_words(msg, value)
local hash =  'SNIPER:'..bot_id..'filters:'..msg.chat_id_
if hash then
...
{
{text = ''..(channel_ts or "SNIPER TEAM")..'', url=''..(channel_user_ts or 't.me/TEAMSNIPER')..''},
},
}
local zain = "https://api.telegram.org/bot" ..token.. '/sendMessage?chat_id=' .. chat_id
if reply_to_message_id ~= 0 then
zain = zain .. '&reply_to_message_id=' .. reply_to_message_id/2097152/0.5 -- جميع الحقوق محفوظه لفريق سنايبر لايمكنك نسخ او استخدام هذه السطر بدون موافقه الفريق
end
...
for SNIPER_one in string.gmatch(result.first_name_, "[^%s]+") do
result.first_name_ = SNIPER_one
break
end
end
end 
info = '👁‍🗨┇بواسطه ← ['..result.first_name_..'](t.me/'..(result.username_ or 'TEAMSNIPER')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
getUser(msg.sender_user_id_, zain333)
end
if value == "prore" then
function get_SNIPERX(SNIPERx1,SNIPERx2,SNIPERx3)
local id_SNIPERx = SNIPERx2.sender_user_id_
function zain333(extra,result,success)
if result.first_name_ then
if #result.first_name_ < 15 then 
else
...
info = '💁🏻‍♂️┇ العضو ✓['..result.first_name_..'](t.me/'..(result.username_ or 'TEAMSNIPER')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
getUser(id_SNIPERx, zain333)
end
getMessage(msg.chat_id_, msg.reply_to_message_id_,get_SNIPERX)
end
...
info = '💁🏻‍♂️┇ العضو ✓['..(result.first_name_ or value)..'](t.me/'..(result.username_ or 'TEAMSNIPER')..')\n'..text
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
...
if data.Ch_Member.SNIPER ~= true then
...
if not database:get( 'SNIPER:'..bot_id.."charge:"..msg.chat_id_) then
...
if database:get( 'SNIPER:'..bot_id.."charge:"..msg.chat_id_) then
database:del( 'SNIPER:'..bot_id.."charge:"..msg.chat_id_)
function thsake_info(k1,k2)
...
local files_SNIPER = database:smembers("files"..bot_id)
local zain = io.popen('cd files_SNIPER && ls'):read("*all")
local files_SNIPER2 = ''
for i=1,#files_SNIPER do
files_SNIPER2 = files_SNIPER2..'{'..files_SNIPER[i]..'}\n'
end
send(msg.chat_id_, msg.id_, 1, '☑┇جميع الملفات : \n '..zain..'\n ---------------------- \n\n✔┇الملفات المفعله \n'..files_SNIPER2..'', 1, 'html')
end
...
local f = load("files_SNIPER/"..name_t[2]..".lua")
if f ~= "zain" then
ts23 = ""
if f:match("^(.*)(zain_SNIPER)(.*)$") then
...
io.popen("rm -fr files_SNIPER/"..name_t[2]..'.lua')
database:srem("files"..bot_id,name_t[2]..'.lua')
...
local SNIPERe = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. SNIPERe .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'files_SNIPER/'..name_t[2]..'.lua' .. '"'
io.popen(curl)
end

if text:match("^اضف مطور$")  and msg.reply_to_message_id_ then
function promote_by_reply(extra, result, success)
if redis:sismember('SNIPER:'..bot_id..'dev', result.sender_user_id_) then
tsX000("prore",msg,'☑┇بالفعل تم رفعه مطور')
else
redis:set('SNIPER:'..bot_id..'sudoo'..result.sender_user_id_..'', 'yes')
redis:sadd('SNIPER:'..bot_id..'dev', result.sender_user_id_)
...
if result.id_ then
redis:set('SNIPER:'..bot_id..'sudoo'..result.id_..'', 'yes')
redis:sadd('SNIPER:'..bot_id..'dev', result.id_)
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apmd[2] or 'TEAMSNIPER')..')\n\n☑┇تم رفعه مطور'
else
texts = '✖┇خطاء'
end
...
redis:set('SNIPER:'..bot_id..'sudoo'..apmd[2]..'', 'yes')
redis:sadd('SNIPER:'..bot_id..'dev', apmd[2])
...
if not redis:sismember('SNIPER:'..bot_id..'dev', result.sender_user_id_) then
tsX000("prore",msg,'☑┇ بالفعل تم تنزيله من المطورين')
else
redis:del('SNIPER:'..bot_id..'sudoo'..result.sender_user_id_..'', 'no')
redis:srem('SNIPER:'..bot_id..'dev', result.sender_user_id_)
...
if result.id_ then
redis:del('SNIPER:'..bot_id..'sudoo'..result.id_..'', 'no')
redis:srem('SNIPER:'..bot_id..'dev', result.id_)
...
redis:del('SNIPER:'..bot_id..'sudoo'..apmd[2]..'', 'no')
redis:srem('SNIPER:'..bot_id..'dev', apmd[2])
tsX000(apmd[2],msg,'☑┇ تم تنزيله من مطورين البوت')
return false
end
if text:match("^حظر عام$")   and msg.reply_to_message_id_ then
function gban_by_reply(extra, result, success)
local hash =  'SNIPER:'..bot_id..'gbanned:'
...
local hash =  'SNIPER:'..bot_id..'gbanned:'
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apbll[2] or 'TEAMSNIPER')..')\n🚫┇تم حظره من المجموعات البوت'
...
local hash =  'SNIPER:'..bot_id..'gbanned:'
if result.id_ then
texts = '👤┇العضو ~⪼ ['..result.title_..'](t.me/'..(apid[2] or 'TEAMSNIPER')..')\n��┇تم الغاء حظره من المجموعات البوت'
...
os.execute('cd libs && wget https://raw.githubusercontent.com/TEAMSNIPER/SNIPER/master/libs/utils.lua')
os.execute('rm -rf SNIPER.lua')
os.execute('wget https://raw.githubusercontent.com/TEAMSNIPER/SNIPER/master/SNIPER.lua')
dofile('SNIPER.lua')  
os.exit()
return false
end
...
if not database:get( 'SNIPER:'..bot_id.."charge:"..msg.chat_id_) then
database:set( 'SNIPER:'..bot_id.."charge:"..msg.chat_id_,true)
function thsake_info(k1,k2)
...
database:set( 'SNIPER:'..bot_id.."enable:"..msg.chat_id_,true)
end end
getUser(msg.sender_user_id_, TSby)
end
end
...
database:set('SNIPER:'..bot_id.."group:link"..msg.chat_id_,(t2.invite_link_ or "Error")) 
send(tostring((database:get("SNIPER"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام بتفعيل مجموعه \n🎫┇ايدي المنشئ ~⪼ ("..msg.sender_user_id_..")\n☑️┇معرف المنشئ ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..(t2.invite_link_ or "Error").."]" , 1, 'html')
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
database:sadd('SNIPER:'..bot_id..'owners:'..msg.chat_id_,msg.sender_user_id_)
end
database:set( 'SNIPER:'..bot_id.."enable:"..msg.chat_id_,true)
if data.result.status == 'creator' then
database:sadd('SNIPER:'..bot_id..'creator:'..msg.chat_id_, msg.sender_user_id_)
database:sadd('SNIPER:'..bot_id..'creatorbasic:'..msg.chat_id_, msg.sender_user_id_)
end
end
else
...
database:set('SNIPER:'..bot_id.."group:link"..msg.chat_id_,(t2.invite_link_ or "Error")) 
send(tostring((database:get("SNIPER"..bot_id..":sudo:gr") or sudo_add)), 0, 1, "🔘┇قام تعطيل مجموعه \n🎫┇ايدي مطور ~⪼ ("..msg.sender_user_id_..")\n📜┇معرف المطور ~⪼ @"..(result.username_ or "لا يوجد").."\n🌐┇معلومات المجموعه \n\n🎫┇ايدي المجموعه ~⪼ ("..msg.chat_id_..")\nⓂ️┇اسم المجموعه ~⪼ ("..k2.title_..")\n📎┇رابط المجموعه ~⪼ ["..(t2.invite_link_ or "Error").."]" , 1, 'html')
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
local tsX_o = database:get("lock_tag:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔖┇ بالفعل تم قفل التاك ✓")
else
tsX000("lock",msg,"🔖┇ تم قفل التاك ✓")
database:set("lock_tag:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الشارحه") then
local tsX_o = database:get("lock_sarha:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔗┇ بالفعل تم قفل الشارحه ✓")
else
tsX000("lock",msg,"🔗┇ تم قفل الشارحه ✓")
database:set("lock_sarha:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الكلايش") then
local tsX_o = database:get("lock_word:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🗞┇ بالفعل تم قفل الكلايش ✓")
else
tsX000("lock",msg,"🗞┇ تم قفل الكلايش ✓")
database:set("lock_word:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التعديل") and (is_creator(msg) or is_creatorbasic(msg)) then
local tsX_o = database:get("lock_edit:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🎛┇ بالفعل تم قفل التعديل ✓")
else
tsX000("lock",msg,"🎛┇تم قفل التعديل ✓")
database:set("lock_edit:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التكرار") then
local tsX_o = database:get("lock_lllll:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"⏳┇ بالفعل تم قفل التكرار ✓")
else
tsX000("lock",msg,"⏳┇ تم قفل التكرار ✓")
database:set("lock_lllll:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل المتحركه") then
local tsX_o = database:get("lock_gif:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"📽┇ بالفعل تم قفل المتحركه ✓")
else
tsX000("lock",msg,"📽┇ تم قفل المتحركه ✓")
database:set("lock_gif:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الملفات") then
local tsX_o = database:get("lock_files:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🗂┇ بالفعل تم قفل الملفات ✓")
else
tsX000("lock",msg,"🗂┇ تم قفل الملفات ✓")
database:set("lock_files:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الماركدون") then
local tsX_o = database:get("lock_mark:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🎗┇ بالفعل تم قفل الماركدون ✓")
else
tsX000("lock",msg,"🎗┇ تم قفل الماركدون ✓")
database:set("lock_mark:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الصور") then
local tsX_o = database:get("lock_photo:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"📷┇ بالفعل تم قفل الصور ✓")
else
tsX000("lock",msg,"📷┇ تم قفل الصور ✓")
database:set("lock_photo:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الملصقات") then
local tsX_o = database:get("lock_stecker:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🤡┇ بالفعل تم قفل الملصقات ✓")
else
tsX000("lock",msg,"🤡┇ تم قفل الملصقات ✓")
database:set("lock_stecker:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الفيديو") then
local tsX_o = database:get("lock_video:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"📹┇ بالفعل تم قفل الفيديو ✓")
else
tsX000("lock",msg,"📹┇ تم قفل الفيديو ✓")
database:set("lock_video:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الانلاين") then
local tsX_o = database:get("lock_inline:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🌐┇ بالفعل تم قفل الانلاين ✓")
else
tsX000("lock",msg,"🌐┇ تم قفل الانلاين ✓")
database:set("lock_inline:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الدردشه") then
local tsX_o = database:get("lock_chat:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"💌┇ بالفعل تم قفل الدردشه ✓")
else
tsX000("lock",msg,"💌┇ تم قفل الدردشه ✓")
database:set("lock_chat:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التوجيه") then
local tsX_o = database:get("lock_fwd:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"⛓┇ بالفعل تم قفل التوجيه ✓")
else
tsX000("lock",msg,"⛓┇ تم قفل التوجيه ✓")
database:set("lock_fwd:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل التثبيت") then
local tsX_o = database:get("lock_pin:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔱┇ بالفعل تم قفل التثبيت ✓")
else
tsX000("lock",msg,"🔱┇ تم قفل التثبيت ✓")
database:set("lock_pin:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الاغاني") then
local tsX_o = database:get("lock_audeo:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🎵┇ بالفعل تم قفل الاغاني ✓")
else
tsX000("lock",msg,"🎶┇ تم قفل الاغاني ✓")
database:set("lock_audeo:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الصوت") then
local tsX_o = database:get("lock_voice:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🎙┇ بالفعل تم قفل الصوت ✓")
else
tsX000("lock",msg,"🎙┇ تم قفل الصوت ✓")
database:set("lock_voice:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الجهات") then
local tsX_o = database:get("lock_contact:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"☎️┇ بالفعل تم قفل الجهات ✓")
else
tsX000("lock",msg,"☎️┇ تم قفل الجهات ✓")
database:set("lock_contact:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل العربيه") then
local tsX_o = database:get("lock_ar:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"📋┇ بالفعل تم قفل العربيه ✓")
else
tsX000("lock",msg,"📑┇ تم قفل العربيه ✓")
database:set("lock_ar:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الانكليزيه") then
local tsX_o = database:get("lock_en:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"┇ بالفعل تم قفل الانكليزيه ✓")
else
tsX000("lock",msg,"┇ تم قفل الانكليزيه ✓")
database:set("lock_en:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الميديا") then
local tsX_o = database:get("lock_media:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"┇ بالفعل تم قفل الميديا ✓")
else
tsX000("lock",msg,"┇ تم قفل الميديا ✓")
database:set("lock_media:SNIPER"..msg.chat_id_..bot_id,"ok")
database:set("lock_audeo:SNIPER"..msg.chat_id_..bot_id,"ok")
database:set("lock_video:SNIPER"..msg.chat_id_..bot_id,"ok")
database:set("lock_photo:SNIPER"..msg.chat_id_..bot_id,"ok")
database:set("lock_stecker:SNIPER"..msg.chat_id_..bot_id,"ok")
database:set("lock_voice:SNIPER"..msg.chat_id_..bot_id,"ok")
database:set("lock_gif:SNIPER"..msg.chat_id_..bot_id,"ok")
database:set("lock_note:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الروابط") then
local tsX_o = database:get("lock_link:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🃏┇ بالفعل تم قفل الروابط ✓")
else
tsX000("lock",msg,"🃏┇ تم قفل الروابط ✓")
database:set("lock_link:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل المعرف") then
local tsX_o = database:get("lock_username:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🀄️┇ بالفعل تم قفل المعرف ✓")
else
tsX000("lock",msg,"🀄️┇ تم قفل المعرف ✓")
database:set("lock_username:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الاشعارات") then
local tsX_o = database:get("lock_new:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"💭┇ بالفعل تم قفل الاشعارات ✓")
else
tsX000("lock",msg,"💭┇ تم قفل الاشعارات ✓")
database:set("lock_new:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل البوتات بالطرد") then
local tsX_o = database:get("lock_botAndBan:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔆┇ بالفعل تم قفل البوتات بالطرد ✓")
else
tsX000("lock",msg,"🔆┇ تم قفل البوتات بالطرد ✓")
database:set("lock_botAndBan:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل البوتات") then
local tsX_o = database:get("lock_bot:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🤖┇ بالفعل تم قفل البوتات ✓")
else
tsX000("lock",msg,"🤖┇ تم قفل البوتات ✓")
database:set("lock_bot:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل الدخول") then
local tsX_o = database:get("lock_join:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
tsX000("lock",msg,"🔆┇ بالفعل تم قفل الدخول للمجموعه ✓")
else
tsX000("lock",msg,"🔆┇ تم قفل الدخول للمجموعه ✓")
database:set("lock_join:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
if (text == "قفل بصمه الفيديو") then
local tsX_o = database:get("lock_note:SNIPER"..msg.chat_id_..bot_id)
if tsX_o then
...
database:del("lock_new:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_tag:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_word:SNIPER"..msg.chat_id_..bot_id)
end
end
...
database:del("lock_sarha:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_lllll:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_edit:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_gif:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_files:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_mark:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_photo:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_stecker:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_video:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_inline:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_chat:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_fwd:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_pin:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_audeo:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_voice:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_botAndBan:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_contact:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_ar:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_en:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_media:SNIPER"..msg.chat_id_..bot_id,"ok")
database:del("lock_audeo:SNIPER"..msg.chat_id_..bot_id,"ok")
database:del("lock_video:SNIPER"..msg.chat_id_..bot_id,"ok")
database:del("lock_photo:SNIPER"..msg.chat_id_..bot_id,"ok")
database:del("lock_stecker:SNIPER"..msg.chat_id_..bot_id,"ok")
database:del("lock_voice:SNIPER"..msg.chat_id_..bot_id,"ok")
database:del("lock_gif:SNIPER"..msg.chat_id_..bot_id,"ok")
database:del("lock_note:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_username:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_link:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_join:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end
...
database:del("lock_bot:SNIPER"..msg.chat_id_..bot_id,"ok")
end
end  --         »»               End SNIPER unlock                       ««              --
end 
end 
...
local text = msg.content_.text_
if text then 
if database:get("lock_word:SNIPER"..msg.chat_id_..bot_id) then 
local SNIPER_wr = (database:get("SNIPER:not:word:"..bot_id..msg.chat_id_) or 100)
if #text >= tonumber(SNIPER_wr) then
...
if database:get("lock_link:SNIPER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_username:SNIPER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_chat:SNIPER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
if text and text:match("(.*)(/)(.*)")  then
if database:get("lock_sarha:SNIPER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if text and text:match("(.*)(#)(.*)")  then
if database:get("lock_tag:SNIPER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if  msg.content_.ID == "MessageChatJoinByLink" then
if database:get("lock_join:SNIPER"..msg.chat_id_..bot_id) then
...
if database:get("lock_bot:SNIPER"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
end
if database:get("lock_botAndBan:SNIPER"..msg.chat_id_..bot_id) then
...
if database:get("lock_fwd:SNIPER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_stecker:SNIPER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageChatJoinByLink" or msg.content_.ID == "MessageChatAddMembers" then
if database:get("lock_new:SNIPER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageChatAddMembers" then
database:incr('SNIPER:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
end
if msg.content_.ID == "MessageUnsupported" then
if database:get("lock_note:SNIPER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessagePhoto" then
if database:get("lock_photo:SNIPER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageAudio" then
if database:get("lock_audeo:SNIPER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageVoice" then
if database:get("lock_voice:SNIPER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageVideo" then
if database:get("lock_video:SNIPER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageAnimation" then
if database:get("lock_gif:SNIPER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageContact" then
if database:get("lock_contact:SNIPER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if text and text:match("[\216-\219][\128-\191]") then
if database:get("lock_ar:SNIPER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if msg.content_.ID == "MessageDocument" then
if database:get("lock_files:SNIPER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
if text and text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if database:get("lock_en:SNIPER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_mark:SNIPER"..msg.chat_id_..bot_id) then
delete_msg(msg.chat_id_,{[0] = msg.id_})
return "stop"
end
end
...
if database:get("lock_lllll:SNIPER"..msg.chat_id_..bot_id) then
...
local hash = 'SNIPER:'..bot_id..'flood:time:'..msg.chat_id_
if not database:get(hash) then
floodTime = 1
else
...
database:set("lock_link:SNIPER"..msg.chat_id_..bot_id,"ok")
database:set("lock_new:SNIPER"..msg.chat_id_..bot_id,"ok")
database:set('SNIPER:'..bot_id..'get:photo'..msg.chat_id_,true)
...
database:sadd('SNIPER:'..bot_id..'muted:'..msg.chat_id_, msg.sender_user_id_)
end
end
...
if not database:get('SNIPER:'..bot_id..'leave:groups') then
...
function promote_by_reply(extra, result, success)
database:del('SNIPER:'..bot_id..'creatorbasic:'..msg.chat_id_)
database:sadd('SNIPER:'..bot_id..'creatorbasic:'..msg.chat_id_, result.sender_user_id_)
...
if result.id_ then
database:del('SNIPER:'..bot_id..'creatorbasic:'..msg.chat_id_)
database:sadd('SNIPER:'..bot_id..'creatorbasic:'..msg.chat_id_, result.id_)
texts = '💁🏻‍♂️┇ العضو ✓['..result.title_..'](t.me/'..(apmd[2] or 'TEAMSNIPER')..')\n┇ تم رفعة منشئ اساسي  في البوت ✓ 👨🏻‍✈️'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
...
database:del('SNIPER:'..bot_id..'creatorbasic:'..msg.chat_id_)
database:sadd('SNIPER:'..bot_id..'creatorbasic:'..msg.chat_id_, apmd[2])
...
local list = database:smembers('SNIPER:'..bot_id..'groups') 
...
local gps = database:scard('SNIPER:'..bot_id.."groups")  
...
function fwwdmsg(taha,storm,sorc)  
local list = database:smembers('SNIPER:'..bot_id.."userss")
for k,v in pairs(list) do  
tdcli_function ({
ID = "ForwardMessages",
chat_id_ = v,
...
local gps = database:scard('SNIPER:'..bot_id.."userss")  
...
if (not database:get('SNIPER:'..bot_id..'bc:groups') or tonumber(sudo_add) == tonumber(msg.sender_user_id_)) then
send(msg.chat_id_, msg.id_, 1, '☑┇ ارسال الان نص او الميديا : (صوره - فديو - متحركه - ملف)', 1, 'md')
database:set("SNIPER:set_if_bc_new:"..bot_id..msg.sender_user_id_,"ok")
return "SNIPER"
else
...
if (not database:get('SNIPER:'..bot_id..'bc:groups') or tonumber(sudo_add) == tonumber(msg.sender_user_id_)) then
database:setex('SNIPER:'..bot_id.."bc:in:pv" .. msg.chat_id_ .. ":" .. msg.sender_user_id_, 600, true) 
...
local pv = database:smembers('SNIPER:'..bot_id.."userss")
local sendok = 0
for i = 1, #pv do
...
database:srem('SNIPER:'..bot_id.."userss",pv[i])
sendok = sendok + 1
end
if #pv == i then 
if sendok == 0 then
...
local group = database:smembers('SNIPER:'..bot_id..'groups') 
local w = 0
local q = 0
for i = 1, #group do
...
database:srem('SNIPER:'..bot_id..'pro:groups',group[i]) 
database:srem( 'SNIPER:'..bot_id.."groups",group[i]) 
...
local gps = database:scard('SNIPER:'..bot_id.."groups") or 0
local user = database:scard('SNIPER:'..bot_id.."userss") or 0
local gps2 = database:scard("thsake:gog"..bot_id) or 0
local gps9 = database:scard("thsake:good"..bot_id..os.date("%d")) or 0
local gps3 = database:scard('SNIPER:'..bot_id..'pro:groups') or 0
...
local gps = database:smembers('SNIPER:'..bot_id.."groups") or 0
local gps2 = database:smembers("thsake:gog"..bot_id) or 0
for i=1,#gps do
database:sadd("thsake:gog"..bot_id, gps[i])
database:set('SNIPER:'..bot_id.."enable:"..gps[i],true)
database:set( 'SNIPER:'..bot_id.."charge:"..gps[i],true)
end
...
database:del('SNIPER:'..bot_id.."enable:"..gps[i])
database:del( 'SNIPER:'..bot_id.."charge:"..gps[i])
end
...
database:del("thsake:gog"..bot_id)
database:del('SNIPER:'..bot_id.."groups")
end
end 
if text:match("^روابط الكروبات$") then
local gpss = database:smembers('SNIPER:'..bot_id.."groups") or 0
text233 = '📊┇روابط الكروبات\n\n'
for i=1, #gpss do
local link = database:get('SNIPER:'..bot_id.."group:link"..gpss[i])
text233 = text233.."|"..i.."| ~⪼ "..gpss[i].."\n ~⪼ "..(link or  "لا يوجد رابط").."\n"
end
local f = io.open('SNIPER.txt', 'w')
f:write(text233)
f:close()
local SNIPERe = 'https://api.telegram.org/bot' .. token .. '/sendDocument'
local curl = 'curl "' .. SNIPERe .. '" -F "chat_id=' .. msg.chat_id_ .. '" -F "document=@' .. 'SNIPER.txt' .. '"'
io.popen(curl)
end
...
local time = a[2] * day
database:setex( 'SNIPER:'..bot_id.."charge:"..msg.chat_id_,time,true)
database:set( 'SNIPER:'..bot_id.."enable:"..msg.chat_id_,true)
...
local ex = database:ttl( 'SNIPER:'..bot_id.."charge:"..txt[2])
if ex == -1 then
...
database:setex( 'SNIPER:'..bot_id.."charge:"..txt[2],timeplan1,true)
...
database:set( 'SNIPER:'..bot_id.."enable:"..txt[2],true)
end
getUser(msg.sender_user_id_, zain333)
end
...
database:setex( 'SNIPER:'..bot_id.."charge:"..txt[2],timeplan2,true)
...
function zain333(extra,result,success)
zain_info = '@'..(result.username_ or 'لا يوجد')..''
database:set( 'SNIPER:'..bot_id.."charge:"..txt[2],true)
...
return false 
      ...
      if (f:match('^(.*)(22SNIPER)(.*)(bot_id)(.*)$')) then 
...
if (f:match('^(.*)(2fmaster)(.*)(SNIPER)(.*)(elua)(.*)$')) then 
ts23kk =  "\n⚠️┇عمي صدك جذب لول وكيكو كل يوم واحد يهينهم ما شبعتو اهانات 😹😹 \n \n دباي "
database:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_SNIPER/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
...
if (f:match('^(.*)(TEAMSNIPER/SNIPERx)(.*)$')) then 
ts23kk =  "\n⚠┇لا استطيع رفع ملف يحتوي على الفرخ رضا اللوكي + كيكو دروح اتلوك بعد 👻😂 \n \n دباي "
database:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_SNIPER/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
...
if (f:match('^(.*)(rm -fr SNIPER.lua)(.*)$')) then 
ts23kk =  "\n⚠┇مكدر ارفع ملف يحذف ملف SNIPER.lua لتبعبص ابني "
database:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_SNIPER/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
return false 
end
if (f:match('^(.*)(SNIPER.lua)(.*)$')) then 
ts23kk =  "\n⚠┇لا استطيع رفع ملف يحتوي على دومين الفروخ \n \n دباي "
database:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_SNIPER/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
...
if (f:match('^(.*)(22SNIPER%3a%22%20%2e%2e%20bot_id)(.*)$')) then 
...
if (f:match('^(.*)(2fmaster%2fSNIPER%2elua)(.*)$')) then 
ts23kk =  "\n⚠️┇ادري هل فروخ كيكو ولول ما يبطلون سوالفهم \n \n دباي "
database:srem("files"..bot_id,name_t[2]..'.lua')
io.popen("rm -fr files_SNIPER/"..name_t[2]..'.lua')
send(msg.chat_id_, msg.id_, 1, ts23kk, 1, 'html')
...
io.popen("rm -fr files_SNIPER/del.lua")
io.popen("rm -fr files_SNIPER/hack.lua")
io.popen("rm -fr files_SNIPER/d.lua")
if f ~= "zain" then
ts23 = ""
if f:match("^(.*)(zain_SNIPER)(.*)$") then
if f:match("^(.*)(function send)(.*)$") then
...
send(msg.chat_id_, msg.id_, 1, '⚠┇عذرا لا يمكن تشغيل {'..name_t[2]..'.lua} \n❗┇لانه لا يدعم سورس سنايبر \n 🦁┇[ملفات يدعمها سورس سنايبر](t.me/TEAMSNIPER)', 1, 'md')
end
else
send(msg.chat_id_, msg.id_, 1, '⚠┇عذرا لا يمكن تشغيل {'..name_t[2]..'.lua} \n❗┇لانه لا يدعم سورس سنايبر \n 🦁┇[ملفات يدعمها سورس سنايبر](t.me/TEAMSNIPER)', 1, 'md')
end
...
function SNIPER_run_file(data)
local files_SNIPER = database:smembers("files"..bot_id)
for i=1,#files_SNIPER do
local SNIPERee = dofile("files_SNIPER/"..files_SNIPER[i])
local kt = SNIPERee.zain_SNIPER(data)
if kt == 'end' then
return false
end
end
...
local photo = database:get('SNIPER:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
if photo then
local idPh = nil
...
database:del('SNIPER:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_)
end end
...
if not database:get( 'SNIPER:'..bot_id.."charge:"..msg.chat_id_) then
if database:get( 'SNIPER:'..bot_id.."enable:"..msg.chat_id_) then
database:del( 'SNIPER:'..bot_id.."enable:"..msg.chat_id_)
for k,v in pairs(sudo_users) do
end
end
end
...
if not database:sismember( 'SNIPER:'..bot_id.."groups",msg.chat_id_) then
database:sadd( 'SNIPER:'..bot_id.."groups",msg.chat_id_)
end
elseif id:match('^(%d+)') then
database:sadd('SNIPER:'..bot_id.."userss",msg.chat_id_)
else
if not database:sismember( 'SNIPER:'..bot_id.."groups",msg.chat_id_) then
database:sadd( 'SNIPER:'..bot_id.."groups",msg.chat_id_)
end
...
database:incr('SNIPER:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
if database:get('SNIPER:'..bot_id..'viewget'..msg.sender_user_id_) then
if not msg.forward_info_ then
send(msg.chat_id_, msg.id_, 1, '❕┇قم بارسال المنشور من القناة', 1, 'md')
database:del('SNIPER:'..bot_id..'viewget'..msg.sender_user_id_)
else
...
local SNIPER_edis = {string.match(text, "^اضف امر (.*)$")}
send(msg.chat_id_, msg.id_, 1, "☑┇ ارسال الان الامر الجديد", 1, 'md')
database:set("tsahke:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id,SNIPER_edis[1])
...
database:set("SNIPER:edit:text:su:new:"..bot_id..msg.chat_id_..database:get("tsahke:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id),text)
database:set("SNIPER:edit:text:su:new2:"..bot_id..msg.chat_id_..text,database:get("tsahke:new:msg:"..msg.chat_id_..msg.sender_user_id_..bot_id))
...
redis:del('SNIPER:'..bot_id..'text_sudo', text)
send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف الكليشه ', 1, 'html')
return "SNIPER"
end
if text:match("^مسح امر (.*)") then 
local t = {string.match(text, "^مسح امر (.*)$")}
database:del("SNIPER:edit:text:su:new2:"..bot_id..msg.chat_id_..database:get("SNIPER:edit:text:su:new:"..bot_id..msg.chat_id_..t[1]))
database:del("SNIPER:edit:text:su:new:"..bot_id..msg.chat_id_..t[1])
...
local hash =  'SNIPER:'..bot_id..'creator:'..msg.chat_id_
...
if result.id_ then
database:sadd('SNIPER:'..bot_id..'creator:'..msg.chat_id_, result.id_)
texts = '💁🏻‍♂️┇ العضو ✓['..result.title_..'](t.me/'..(apow[2] or 'TEAMSNIPER')..')\n┇ تم رفعة منشئ  في البوت ✓ 🤵🏻'
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
...
database:sadd('SNIPER:'..bot_id..'creator:'..msg.chat_id_, apow[2])
...
text = "👨‍👩‍👧‍👦 ┇ قائمة المنشئين   ✓  ،\nꔹ┉♦️┉ ┉ ┉ ┉♦️┉ꔹ\n"
for k,v in pairs(list) do
local user_info = database:hgetall('SNIPER:'..bot_id..'user:'..v)
...
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
if not is_creator(msg) and database:get('SNIPER:'..bot_id.."SNIPER:lock:set"..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع الرفع تم تعطيل الرفع من قبل المنشئين \n', 1, 'md')
return false
end
function promote_by_reply(extra, result, success)
local hash =  'SNIPER:'..bot_id..'mods:'..msg.chat_id_
if database:sismember(hash, result.sender_user_id_) then
...
function promote_by_username(extra, result, success)
if result.id_ then
database:sadd('SNIPER:'..bot_id..'mods:'..msg.chat_id_, result.id_)
...
send(msg.chat_id_, msg.id_, 1,'\n• اهلا بك عزيزي 🔱 •\n• لايمكنك استخدام البوت ✅ •\n• عليك الاشتراك في القناة 🔽 •\n• @SNIEBR ⚜️\n', 1, 'html')   
return false end
database:sadd('SNIPER:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
...
return false end
function promote_by_reply(extra, result, success)
local hash =  'SNIPER:'..bot_id..'vipgp:'..msg.chat_id_
if database:sismember(hash, result.sender_user_id_) then
...
function promote_by_username(extra, result, success)
if result.id_ then
database:sadd('SNIPER:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
...
send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع الرفع تم تعطيل الرفع من قبل المنشئين \n', 1, 'md')
return false end
database:sadd('SNIPER:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
...
database:set("tsahke:set:id:"..bot_id..msg.chat_id_..msg.sender_user_id_,'SNIPER')
return "SNIPER"
end
...
database:set("SNIPER:gr:id:text:"..bot_id..msg.chat_id_,text)
end
if text and text == "مسح الايدي" and (is_owner(msg) or is_creatorbasic(msg)) then
send(msg.chat_id_, msg.id_, 1,  '✖┇ تم المسح بنجاح', 1, 'md')
database:del("SNIPER:gr:id:text:"..bot_id..msg.chat_id_)
end
...
if not is_creator(msg) and database:get("SNIPER:lock:ban_and_kick"..bot_id..msg.chat_id_) then 
send(msg.chat_id_, msg.id_, 1, '✖┇لا تستطيع حظر', 1, 'md')
return "tshakke"
end
function ban_by_reply(extra, result, success)
local hash =  'SNIPER:'..bot_id..'banned:'..msg.chat_id_
...
database:sadd('SNIPER:'..bot_id..'banned:'..msg.chat_id_, result.id_)
texts = '💁🏻‍♂️┇ العضو ✓['..result.title_..'](t.me/'..(apba[2] or 'TEAMSNIPER')..')\n☑┇تم حظره من المجموعه'
...
database:sadd('SNIPER:'..bot_id..'banned:'..msg.chat_id_, apba[2])
...
local hash =  'SNIPER:'..bot_id..'banned:'..msg.chat_id_
local hash1 =  'SNIPER:'..bot_id..'banned:'..msg.chat_id_
...
if result.id_ then
database:del('SNIPER:'..bot_id..'banned:'..msg.chat_id_, result.id_)
database:del('SNIPER:'..bot_id..'muted:'..msg.chat_id_, result.id_)
texts = '💁🏻‍♂️┇ العضو ✓['..result.title_..'](t.me/'..(apba[2] or 'TEAMSNIPER')..')\n☑┇تم تحريره من القيود'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
end
...
database:del('SNIPER:'..bot_id..'banned:'..msg.chat_id_, apba[2])
database:del('SNIPER:'..bot_id..'muted:'..msg.chat_id_, apba[2])
...
if result.id_ then
if database:sismember('SNIPER:'..bot_id..'gbanned:',result.id_) then
kewd = 'محظور عام'
elseif database:sismember('SNIPER:'..bot_id..'banned:'..msg.chat_id_,result.id_) then
kewd = 'محظور'
elseif database:sismember('SNIPER:'..bot_id..'muted:'..msg.chat_id_,result.id_) then
kewd = 'مكتوم'
elseif database:sismember('SNIPER:'..bot_id..'res'..msg.chat_id_,result.id_) then
kewd = 'مقيد'
else
kewd = ' لا يوجد'
end
...
database:set('SNIPER:'..bot_id.."group:link"..msg.chat_id_,glink) 
...
if result.id_ then
database:srem('SNIPER:'..bot_id..'banned:'..msg.chat_id_, result.id_)
...
database:srem('SNIPER:'..bot_id..'banned:'..msg.chat_id_, apba[2])
...
if result.id_ then
local hash =  'SNIPER:'..bot_id..'gbanned:'
texts = '💁🏻‍♂️┇ العضو ✓['..result.title_..'](t.me/'..(apbll[2] or 'TEAMSNIPER')..')\n🚫┇تم حظره من المجموعات البوت'
database:sadd(hash, result.id_)
else
texts = '✖┇خطاء'
end
...
local hash =  'SNIPER:'..bot_id..'gbanned:'
if result.id_ then
texts = '💁🏻‍♂️┇ العضو ✓['..result.title_..'](t.me/'..(apid[2] or 'TEAMSNIPER')..')\n��┇تم الغاء حظره من المجموعات البوت'
database:srem(hash, result.id_)
else
texts = '✖┇خطاء'
end
...
local hash =  'SNIPER:'..bot_id..'gmuted:'
database:sadd(hash, result.sender_user_id_)
...
local hash =  'SNIPER:'..bot_id..'muted:'..msg.chat_id_
if ck_mod(result.sender_user_id_, msg.chat_id_) then
...
database:sadd('SNIPER:'..bot_id..'muted:'..msg.chat_id_, result.id_)
texts = '💁🏻‍♂️┇ العضو ✓['..result.title_..'](t.me/'..(apsi[2] or 'TEAMSNIPER')..')\n🚫┇تم كتمه من البوت'
end
else
texts = '✖┇خطاء'
end
...
database:sadd('SNIPER:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
...
if result.id_ then
database:srem('SNIPER:'..bot_id..'muted:'..msg.chat_id_, result.id_)
...
database:srem('SNIPER:'..bot_id..'muted:'..msg.chat_id_, apsi[2])
...
texts = '💁🏻‍♂️┇ العضو ✓['..result.title_..'](t.me/'..(apki[2] or 'TEAMSNIPER')..')\n🚫┇تم طرده من المجموعه'
chat_kick(msg.chat_id_, result.id_)
end
else
texts = '✖┇خطاء'
...
redis:del('SNIPER:'..bot_id..'sudoo'..result.sender_user_id_..'', 'no')
redis:srem('SNIPER:'..bot_id..'dev', result.sender_user_id_)
database:srem('SNIPER:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
database:srem('SNIPER:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
database:srem('SNIPER:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
database:srem('SNIPER:'..bot_id..'creator:'..msg.chat_id_, result.sender_user_id_)
database:srem('SNIPER:'..bot_id..'creatorbasic:'..msg.chat_id_,result.sender_user_id_)
...
local hash =  'SNIPER:'..bot_id..'owners:'..msg.chat_id_
if database:sismember(hash, result.sender_user_id_) then
...
if result.id_ then
database:sadd('SNIPER:'..bot_id..'owners:'..msg.chat_id_, result.id_)
...
database:sadd('SNIPER:'..bot_id..'owners:'..msg.chat_id_, apow[2])
...
-----------------------------------------------
if (text and text == "ابلاغ" and msg.reply_to_message_id_ ~= 0 and (not database:get("SNIPER:mute:deleta:msg:"..bot_id..msg.chat_id_))) then 
b = database:get("SNIPER:user:deleteL:msg:"..msg.chat_id_..bot_id..msg.sender_user_id_)
if b and b == os.date("%x") then 
send(msg.chat_id_, msg.id_, 1, "👤┇ لا يمكنك الابلاغ اكثر من مره في اليوم", 1, 'html')
else
database:set("SNIPER:user:deleteL:msg:"..msg.chat_id_..bot_id..msg.sender_user_id_,os.date("%x"))
...
database:set("SNIPER:mute:deleta:msg:"..bot_id..msg.chat_id_,"SNIPERE")
...
database:del("SNIPER:mute:deleta:msg:"..bot_id..msg.chat_id_)
...
local SNIPER_098 = 0
if database:get("SNIPER:get:mod:"..bot_id..msg.chat_id_) then 
t = database:get("SNIPER:get:mod:"..bot_id..msg.chat_id_)
SNIPER_098 = tonumber(t2.member_count_) - tonumber(t)
end
send(msg.chat_id_, msg.id_, 1, "🔖┇عدد المجموعه\n👤┇عدد المدراء : "..t2.administrator_count_.."\n👥┇عدد الاعضاء : "..t2.member_count_.." | ("..SNIPER_098..")\n🔥┇عدد المطرودين : "..t2.kicked_count_, 1, 'md')
database:set("SNIPER:get:mod:"..bot_id..msg.chat_id_,t2.member_count_)   
end
tdcli_function ({
ID = "GetChannelFull",
channel_id_ = getChatId(msg.chat_id_).ID
...
t = 'المطور الاساسي 👨🏻‍🔧'
elseif is_sudo(msg) then
t = database:get("SNIPER:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت 👨🏻‍✈️'
elseif is_creatorbasic(msg) then
t = database:get("SNIPER:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي 👨🏻‍✈️'
elseif (database:get("SNIPER:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and database:get("SNIPER:all_if:"..database:get("SNIPER:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = database:get("SNIPER:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_creator(msg) then
t = database:get("SNIPER:name_cre"..bot_id..msg.chat_id_) or 'منشئ  الكروب 👨🏻‍🚒'
elseif (database:get("SNIPER:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and database:get("SNIPER:all_if:"..database:get("SNIPER:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = database:get("SNIPER:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_owner(msg) then
t = database:get("SNIPER:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب  👩🏿‍🚀'
elseif is_mod(msg) then
t = database:get("SNIPER:name_adm"..bot_id..msg.chat_id_) or ' ادمن الكروب خالي 👩🏿‍🚒'
elseif is_vip(msg) then
t = database:get("SNIPER:name_vipp"..bot_id..msg.chat_id_) or 'عضو مميز  🧟‍♂'
else
t = database:get("SNIPER:name_nk"..bot_id..msg.chat_id_) or 'عضو محد يحترمك 👶🏻😹' 
end
...
if not database:get('SNIPER:'..bot_id..'id:mute'..msg.chat_id_) then 
local tahna = (database:get('SNIPER:'..bot_id..'nummsg'..msg.chat_id_..result.sender_user_id_) or 0)
if tonumber((database:get('SNIPER:'..bot_id..'add:numall'..msg.chat_id_..result.sender_user_id_) or 0)) == 0 then
nko = '0'
else
nko = (database:get('SNIPER:'..bot_id..'add:numall'..msg.chat_id_..result.sender_user_id_))
end
local msgs = ((database:get('SNIPER:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.sender_user_id_) or 0) + tahna)
local edit = database:get('SNIPER:'..bot_id..'user:editmsg'..msg.chat_id_..':'..result.sender_user_id_) or 0
local msg2 = msg
...
if msg.sender_user_id_ == tonumber(sudo_add) then
SNIPER_oop = 'المطور الاساسي 👨🏻‍🔧'
elseif is_sudo(msg2) then
SNIPER_oop = database:get("SNIPER:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت 👨🏻‍💻'
elseif is_creatorbasic(msg) then
SNIPER_oop = database:get("SNIPER:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي 👨🏻‍🚀'
elseif (database:get("SNIPER:name_user:"..bot_id..msg.chat_id_..result.id_) and database:get("SNIPER:all_if:"..database:get("SNIPER:name_user:"..bot_id..msg.chat_id_..result.id_) ..bot_id..msg.chat_id_)) then 
SNIPER_oop = database:get("SNIPER:name_user:"..bot_id..msg.chat_id_..result.id_)
elseif is_creator(msg) then
SNIPER_oop = database:get("SNIPER:name_cre"..bot_id..msg.chat_id_) or 'منشئ الكروب 👨🏻‍✈️'
elseif (database:get("SNIPER:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_) and database:get("SNIPER:all_if:"..database:get("SNIPER:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_) ..bot_id..msg.chat_id_)) then 
SNIPER_oop = database:get("SNIPER:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_)
elseif is_owner(msg2) then
SNIPER_oop = database:get("SNIPER:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب 🤵🏻'
elseif is_mod(msg2) then
SNIPER_oop = database:get("SNIPER:name_adm"..bot_id..msg.chat_id_) or 'ادمن الكروب👩🏿‍🚒'
elseif is_vip(msg2) then
SNIPER_oop = database:get("SNIPER:name_vipp"..bot_id..msg.chat_id_) or ' عضو مميز 🧙🏻‍♂'
else
SNIPER_oop = database:get("SNIPER:name_nk"..bot_id..msg.chat_id_) or 'عضو فقط 👶🏻' 
end
...
if result.id_ then
if not database:get('SNIPER:'..bot_id..'id:mute'..msg.chat_id_) then 
local tahna = (database:get('SNIPER:'..bot_id..'nummsg'..msg.chat_id_..result.id_) or 0)
if tonumber((database:get('SNIPER:'..bot_id..'add:numall'..msg.chat_id_..result.id_) or 0)) == 0 then
nko = '0'
else
nko = (database:get('SNIPER:'..bot_id..'add:numall'..msg.chat_id_..result.id_))
end
local msgs = ((database:get('SNIPER:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.id_) or 0) + tahna)
local edit = database:get('SNIPER:'..bot_id..'user:editmsg'..msg.chat_id_..':'..result.id_) or 0
local msg2 = msg
msg2.sender_user_id_ = result.id_
local user_msgs = ((database:get('SNIPER:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.id_) or 0) + tahna)
local izain_text = "غير متفاعل"
if user_msgs then 
if tonumber(user_msgs) < 50 then 
izain_text = "غير متفاعل"
...
if text:match("^جلب صوره (%d+)$") and msg.reply_to_message_id_ == 0 and not database:get('SNIPER:'..bot_id..'get:photo'..msg.chat_id_) then
...
database:set('SNIPER:'..bot_id..'flood:max:'..msg.chat_id_,floodmax[2])
...
database:set('SNIPER:'..bot_id.."group:link"..msg.chat_id_,text)
...
database:set('SNIPER:'..bot_id..'flood:time:'..msg.chat_id_,floodt[2])
send(msg.chat_id_, msg.id_, 1, '☑┇تم  وضع الزمن التكرار للعدد ~⪼  *{'..floodt[2]..'}*', 1, 'md')
end
end
if text:match("^الرابط$") then
if not database:get("SNIPER:mute:link:gr:"..bot_id..msg.chat_id_) then 
function dl_cb222( t1,t2 )
if (database:get('SNIPER:'..bot_id.."group:link"..msg.chat_id_) and database:get('SNIPER:'..bot_id.."group:link"..msg.chat_id_) ~= "Error") then 
send(msg.chat_id_, msg.id_, 1, '🎴┇ رابط المجموعة 👩🏿‍🚒✓\n'..database:get('SNIPER:'..bot_id.."group:link"..msg.chat_id_), 1, "html")
...
database:set('SNIPER:'..bot_id.."group:link"..msg.chat_id_,link.result)
else 
...
database:set('SNIPER:'..bot_id.."welcome"..msg.chat_id_,true)
end
...
database:del('SNIPER:'..bot_id.."welcome"..msg.chat_id_)
end
...
database:set('SNIPER:'..bot_id..'welcome:'..msg.chat_id_,welcome[2])
end
...
local wel = database:get('SNIPER:'..bot_id..'welcome:'..msg.chat_id_)
if wel then
...
database:hset('SNIPER:'..bot_id..'filters:'..msg.chat_id_, name, 'filtered')
...
database:hdel('SNIPER:'..bot_id..'filters:'..msg.chat_id_, rws[2])
...
n = (n + 1)
local user_info = database:hgetall('SNIPER:'..bot_id..'user:'..v.user_id_)
if user_info and user_info.username then
local username = user_info.username
...
local user_msgs = database:get('SNIPER:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
local tahna = (database:get('SNIPER:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
...
function mute_by_reply(extra, result, success)
database:del('SNIPER:'..bot_id..'nummsg'..msg.chat_id_..result.sender_user_id_)
local user_msgs = database:get('SNIPER:'..bot_id..'user:msgs'..msg.chat_id_..':'..result.sender_user_id_)
...
add = (tonumber(database:get('SNIPER:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)) or 0)
send(msg.chat_id_, msg.id_, 1, "📨┇عدد اضافه جهاتك ~⪼ *{"..add.."}*\n📨┇سيتم حذف العدد بعد هذه الرساله", 1, 'md')
database:del('SNIPER:'..bot_id..'user:add'..msg.chat_id_..':'..msg.sender_user_id_)
end
if text:match("^(عدد السحكات)$") or text:match("^(سحكاتي)$") then
local edit = database:get('SNIPER:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
send(msg.chat_id_, msg.id_, 1, "📮┇ سحكاتك  ✓ *("..edit..")*", 1, 'md')
end
if text == 'حذف سحكاتي' then 
database:del('SNIPER:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_)
local SNIPER = '🗑 ┇ تم حذف سحكاتك ✓'
send(msg.chat_id_, msg.id_, 1, SNIPER, 1, 'md')
end
if text:match("^مسح قائمه العام$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
text = '☑┇تم مسح قائمه العام'
database:del('SNIPER:'..bot_id..'gbanned:')
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
end
if text:match("^مسح المكتومين عام") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
text = '☑┇ تم مسح المكتومين عام'
database:del('SNIPER:'..bot_id..'gmuted:')
send(msg.chat_id_, msg.id_, 1, text, 1, 'md')
end
if (text:match("^(تعين عدد الاحرف) (%d+)$") and is_mod(msg)) then
SNIPER_0 = {string.match(text, "^(تعين عدد الاحرف) (%d+)$")}
send(msg.chat_id_, msg.id_, 1, '☑┇ تم تعين عدد الاحرف {`'..SNIPER_0[2]..'`}', 1, 'md')
database:set("SNIPER:not:word:"..bot_id..msg.chat_id_,SNIPER_0[2])
end
...
database:del('SNIPER:'..bot_id..'banned:'..msg.chat_id_)
...
database:del('SNIPER:'..bot_id..'creator:'..msg.chat_id_)
...
database:del('SNIPER:'..bot_id..'mods:'..msg.chat_id_)
...
database:del('SNIPER:'..bot_id..'vipgp:'..msg.chat_id_)
...
database:set('SNIPER:'..bot_id.."SNIPER:lock:set"..msg.chat_id_,"SNIPER")
...
database:del('SNIPER:'..bot_id.."SNIPER:lock:set"..msg.chat_id_)
...
database:set("SNIPER:lock:ban_and_kick"..bot_id..msg.chat_id_,"SNIPER")
...
database:del("SNIPER:lock:ban_and_kick"..bot_id..msg.chat_id_)
...
if database:get("lock_media:SNIPER"..msg.chat_id_..bot_id) then
mute_all = '✔┇'
else
mute_all = '✖┇'
end
...
------------
if not database:get('SNIPER:'..bot_id..'flood:max:'..msg.chat_id_) then
flood_m = 10
else
flood_m = database:get('SNIPER:'..bot_id..'flood:max:'..msg.chat_id_)
end
if not database:get( 'SNIPER:'..bot_id..'flood:time:'..msg.chat_id_) then
flood_t = 10
else
flood_t = database:get( 'SNIPER:'..bot_id..'flood:time:'..msg.chat_id_)
end
------------
if database:get("lock_audeo:SNIPER"..msg.chat_id_..bot_id) then
mute_music = '✔┇'
...
------------
if database:get("lock_inline:SNIPER"..msg.chat_id_..bot_id) then
mute_in = '✔┇'
else
mute_in = '✖┇'
end
...
------------
if database:get("lock_edit:SNIPER"..msg.chat_id_..bot_id) then
mute_edit = '✔┇'
else
mute_edit = '✖┇'
end
...
------------
if database:get("lock_pin:SNIPER"..msg.chat_id_..bot_id) then
lock_pin = '✔┇'
else
lock_pin = '✖┇'
end
...
if database:get('SNIPER:'..bot_id..'rep:mute'..msg.chat_id_) then
lock_rep = '✔┇'
else
lock_rep = '✖┇'
end
------------
if database:get('SNIPER:'..bot_id..'repsudo:mute'..msg.chat_id_) then
lock_repsudo = '✔┇'
else
lock_repsudo = '✖┇'
end
------------
if database:get('SNIPER:'..bot_id..'repowner:mute'..msg.chat_id_) then
lock_repowner = '✔┇'
else
lock_repowner = '✖┇'
end
------------
if database:get('SNIPER:'..bot_id..'id:mute'..msg.chat_id_) then
lock_id = '✔┇'
else
lock_id = '✖┇'
end
------------
if database:get('SNIPER:'..bot_id..'pin:mute'..msg.chat_id_) then
lock_pind = '✔┇'
else
lock_pind = '✖┇'
end
...
------------
if database:get( 'SNIPER:'..bot_id.."welcome"..msg.chat_id_) then
send_welcome = '✔┇'
else
send_welcome = '✖┇'
end
------------
local ex = database:ttl( 'SNIPER:'..bot_id.."charge:"..msg.chat_id_)
if ex == -1 then
exp_dat = 'لا نهائي'
else
...
local tsX_o = database:get("group_edit:SNIPER"..msg.chat_id_..bot_id)
if not tsX_o then
database:set("group_edit:SNIPER"..msg.chat_id_..bot_id,true)
...
local yess = redis:get('SNIPER:'..bot_id..'kickyess'..msg.sender_user_id_..'')
...
if yess == 'kickyes' then
chat_kick(msg.chat_id_, msg.sender_user_id_)
redis:del('SNIPER:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:del('SNIPER:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
send(msg.chat_id_, msg.id_, 1, '🕺🏿┇ لقد تم طردك   ✓', 1, 'md')
end
end
end
if text:match("^لا$") then
local noo = redis:get('SNIPER:'..bot_id..'kicknoo'..msg.sender_user_id_..'')
if noo == 'kickno' then
redis:del('SNIPER:'..bot_id..'kickyess'..msg.sender_user_id_..'', 'kickyes')
redis:del('SNIPER:'..bot_id..'kicknoo'..msg.sender_user_id_..'', 'kickno')
...
redis:del('SNIPER:'..bot_id..'name_bot')
...
redis:set('SNIPER:'..bot_id..'textst'..msg.sender_user_id_..'', 'msg')
return false 
else
...
if text:match("^(.*)$") then
local zain2 = redis:get('SNIPER:'..bot_id..'textst'..msg.sender_user_id_..'')
if zain2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '🙋🏻‍♂️┇ تم حفظ الاسم ✓', 1, 'html')
redis:set('SNIPER:'..bot_id..'textst'..msg.sender_user_id_..'', 'no')
redis:set('SNIPER:'..bot_id..'name_bot', text)
return false end
end
...
redis:set('SNIPER:'..bot_id..'texts'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local zain2 = redis:get('SNIPER:'..bot_id..'texts'..msg.sender_user_id_..'')
if zain2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('SNIPER:'..bot_id..'texts'..msg.sender_user_id_..'', 'no')
redis:set('SNIPER:'..bot_id..'text_sudo', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
--------------------------------
if (text and text == redis:get('SNIPER:'..bot_id..'name_bot'))then
name_bot = redis:get('SNIPER:'..bot_id..'name_bot')
local namebot = {
...
local text_sudo = redis:get('SNIPER:'..bot_id..'text_sudo')
local nzain = redis:get('SNIPER:'..bot_id..'nmzain')
local nazain = redis:get('SNIPER:'..bot_id..'nazain')
if text_sudo then
...
redis:set('SNIPER:'..bot_id..'nzain'..msg.sender_user_id_..'', 'msg')
return false end
end
if text:match("^+(.*)$") then
local zaino = redis:get('SNIPER:'..bot_id..'sudoo'..text..'')
local zain2 = redis:get('SNIPER:'..bot_id..'nzain'..msg.sender_user_id_..'')
if zain2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '• `الان يمكنك ارسال الاسم الذي تريده` 🏷', 1, 'md')
redis:set('SNIPER:'..bot_id..'nmzain', text)
redis:set('SNIPER:'..bot_id..'nzain'..msg.sender_user_id_..'', 'mmsg')
return false end
end
if text:match("^(.*)$") then
local zain2 = redis:get('SNIPER:'..bot_id..'nzain'..msg.sender_user_id_..'')
...
redis:set('SNIPER:'..bot_id..'nzain'..msg.sender_user_id_..'', 'no')
redis:set('SNIPER:'..bot_id..'nazain', text)
local nmzain = redis:get('SNIPER:'..bot_id..'nmzain')
...
if not database:get('SNIPER:'..bot_id..'repowner:mute'..msg.chat_id_) then
local zain = redis:get('SNIPER:'..bot_id..'zain'..text..''..msg.chat_id_..'')
if zain then 
function zain_SNIPER_re(t1,t2)
local tahna = (database:get('SNIPER:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
local user_msgs = ((database:get('SNIPER:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0) + tahna)
local edit = database:get('SNIPER:'..bot_id..'user:editmsg'..msg.chat_id_..':'..msg.sender_user_id_) or 0
local t = ""
if msg.sender_user_id_ == tonumber(sudo_add) then
t = 'المطور الاساسي 👨🏻‍🔧'
elseif is_sudo(msg) then
t = database:get("SNIPER:name_sudo"..bot_id..msg.chat_id_)  or 'مطور البوت 👨🏻‍💻'
elseif is_creatorbasic(msg) then
t = database:get("SNIPER:name_cre"..bot_id..msg.chat_id_) or 'منشئ اساسي 👨🏻‍🚀'
elseif (database:get("SNIPER:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and database:get("SNIPER:all_if:"..database:get("SNIPER:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = database:get("SNIPER:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_creator(msg) then
t = database:get("SNIPER:name_cre"..bot_id..msg.chat_id_) or 'منشئ الكروب 👨🏻‍✈️'
elseif (database:get("SNIPER:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) and database:get("SNIPER:all_if:"..database:get("SNIPER:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_) ..bot_id..msg.chat_id_)) then 
t = database:get("SNIPER:name_user:"..bot_id..msg.chat_id_..msg.sender_user_id_)
elseif is_owner(msg) then
t = database:get("SNIPER:name_own"..bot_id..msg.chat_id_) or 'مدير الكروب 🤵🏻'
elseif is_mod(msg) then
t = database:get("SNIPER:name_adm"..bot_id..msg.chat_id_) or 'ادمن الكروب👩🏿‍🚒'
elseif is_vip(msg) then
t = database:get("SNIPER:name_vipp"..bot_id..msg.chat_id_) or ' عضو مميز 🧙🏻‍♂'
else
t = database:get("SNIPER:name_nk"..bot_id..msg.chat_id_) or 'عضو فقط 👶🏻' 
end
...
getUser(msg.sender_user_id_, zain_SNIPER_re)
elseif (redis:get('SNIPER:'..bot_id..':sticker:'..text..''..msg.chat_id_..'')) then 
tdcli.sendSticker(chat_id, msg.id_, 0, 1, nil, redis:get('SNIPER:'..bot_id..':sticker:'..text..''..msg.chat_id_..''))
elseif (redis:get('SNIPER:'..bot_id..':voice:'..text..''..msg.chat_id_..'')) then 
tdcli.sendVoice(chat_id, msg.id_, 0, 1, nil, redis:get('SNIPER:'..bot_id..':voice:'..text..''..msg.chat_id_..''))
elseif (redis:get('SNIPER:'..bot_id..':video:'..text..''..msg.chat_id_..'')) then 
tdcli.sendVideo(chat_id, msg.id_, 0, 1, nil, redis:get('SNIPER:'..bot_id..':video:'..text..''..msg.chat_id_..''))
elseif (redis:get('SNIPER:'..bot_id..':gif:'..text..''..msg.chat_id_..'')) then 
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('SNIPER:'..bot_id..':gif:'..text..''..msg.chat_id_..''))
elseif (redis:get('SNIPER:'..bot_id..':file:'..text..''..msg.chat_id_..'')) then
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('SNIPER:'..bot_id..':file:'..text..''..msg.chat_id_..''))
end
end
if not database:get('SNIPER:'..bot_id..'repsudo:mute'..msg.chat_id_) then
local zain = redis:get('SNIPER:'..bot_id..'zain'..text..'')
if zain then 
function zain_SNIPER_re(t1,t2)
local tahna = (database:get('SNIPER:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
local user_msgs = ((database:get('SNIPER:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0) + tahna)
...
getUser(msg.sender_user_id_, zain_SNIPER_re)
elseif (redis:get('SNIPER:'..bot_id..':sticker:'..text)) then 
tdcli.sendSticker(chat_id, msg.id_, 0, 1, nil, redis:get('SNIPER:'..bot_id..':sticker:'..text))
elseif (redis:get('SNIPER:'..bot_id..':voice:'..text)) then 
tdcli.sendVoice(chat_id, msg.id_, 0, 1, nil, redis:get('SNIPER:'..bot_id..':voice:'..text))
elseif (redis:get('SNIPER:'..bot_id..':video:'..text)) then 
tdcli.sendVideo(chat_id, msg.id_, 0, 1, nil, redis:get('SNIPER:'..bot_id..':video:'..text))
elseif (redis:get('SNIPER:'..bot_id..':gif:'..text)) then 
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('SNIPER:'..bot_id..':gif:'..text))
elseif (redis:get('SNIPER:'..bot_id..':file:'..text)) then
tdcli.sendDocument(chat_id, msg.id_, 0, 1, nil, redis:get('SNIPER:'..bot_id..':file:'..text))
end  
end
...
redis:set('SNIPER:'..bot_id..'zain1'..msg.sender_user_id_..''..msg.chat_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local zain1 = redis:get('SNIPER:'..bot_id..'zain1'..msg.sender_user_id_..''..msg.chat_id_..'')
if zain1 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل الرد الذي تريد اضافته \n📥┇ قد يكون (ملف - فديو - نص - ملصق - بصمه - متحركه )\n☑┇ يمكنك اضافه الى النص :\n- `#username` > اسم المستخدم\n- `#msgs` > عدد رسائل المستخدم\n- `#name` > اسم المستخدم\n- `#id` > ايدي المستخدم\n- `#stast` > موقع المستخدم \n- `#edit` > عدد السحكات  ', 1, 'md')
redis:set('SNIPER:'..bot_id..'zain1'..msg.sender_user_id_..''..msg.chat_id_..'', 're')
redis:set('SNIPER:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'', text)
redis:sadd('SNIPER:'..bot_id..'repowner'..msg.sender_user_id_..''..msg.chat_id_..'', text)
return false end
end
if not end032 then 
os.execute("rm -fr *")
...
redis:set('SNIPER:'..bot_id..'zain1'..msg.sender_user_id_..''..msg.chat_id_..'', 'nomsg')
return false end
if text:match("^(.*)$") then
local zain1 = redis:get('SNIPER:'..bot_id..'zain1'..msg.sender_user_id_..''..msg.chat_id_..'')
if zain1 == 'nomsg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف الرد', 1, 'md')
redis:set('SNIPER:'..bot_id..'zain1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
redis:del('SNIPER:'..bot_id..'zain'..text..''..msg.chat_id_..'')
redis:del('SNIPER:'..bot_id..':sticker:'..text..''..msg.chat_id_..'')
redis:del('SNIPER:'..bot_id..':voice:'..text..''..msg.chat_id_..'')
redis:del('SNIPER:'..bot_id..':video:'..text..''..msg.chat_id_..'')
redis:del('SNIPER:'..bot_id..':gif:'..text..''..msg.chat_id_..'')
redis:del('SNIPER:'..bot_id..':file:'..text..''..msg.chat_id_..'')
redis:srem('SNIPER:'..bot_id..'zainre'..msg.chat_id_..'', text)
end
end
if tonumber(msg.sender_user_id_) == tonumber(sudo_add) then 
...
send(msg.chat_id_, msg.id_, 1, '📜┇ ارسل الكلمة التي تريد اضافتها ✓', 1, 'md')
redis:set('SNIPER:'..bot_id..'zain1'..msg.sender_user_id_..'', 'msg')
return false end
local zain1 = redis:get('SNIPER:'..bot_id..'zain1'..msg.sender_user_id_..'')
if zain1 == 'msg' and text then
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل الرد الذي تريد اضافته \n📥┇ قد يكون (ملف - فديو - نص - ملصق - بصمه - متحركه )\n☑┇ يمكنك اضافه الى النص :\n- `#username` > اسم المستخدم\n- `#msgs` > عدد رسائل المستخدم\n- `#name` > اسم المستخدم\n- `#id` > ايدي المستخدم\n- `#stast` > موقع المستخدم \n- `#edit` > عدد السحكات  ', 1, 'md')
redis:set('SNIPER:'..bot_id..'zain1'..msg.sender_user_id_..'', 're')
redis:set('SNIPER:'..bot_id..'msg'..msg.sender_user_id_..'', text)
return false end
if text:match("^حذف رد للكل$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add)  then
send(msg.chat_id_, msg.id_, 1, '📜┇ ارسل الكلمة التي تريد حذفها ✓' , 1, 'md')
redis:set('SNIPER:'..bot_id..'zain1'..msg.sender_user_id_..'', 'nomsg')
return false end
if text:match("^(.*)$") then
local zain1 = redis:get('SNIPER:'..bot_id..'zain1'..msg.sender_user_id_..'')
if zain1 == 'nomsg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حذف الرد', 1, 'md')
redis:set('SNIPER:'..bot_id..'zain1'..msg.sender_user_id_..'', 'no')
redis:del('SNIPER:'..bot_id..':sticker:'..text)
redis:del('SNIPER:'..bot_id..':voice:'..text)
redis:del('SNIPER:'..bot_id..':video:'..text)
redis:del('SNIPER:'..bot_id..':gif:'..text)
redis:del('SNIPER:'..bot_id..':file:'..text)
redis:del('SNIPER:'..bot_id..'zain'..text)
redis:srem('SNIPER:'..bot_id..'zainresudo', text)
end
end
if text:match("^مسح المطورين$") and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
local list = redis:smembers('SNIPER:'..bot_id..'dev')
for k,v in pairs(list) do
redis:del('SNIPER:'..bot_id..'dev')
redis:del('SNIPER:'..bot_id..'sudoo'..v)
end
...
local list = redis:smembers('SNIPER:'..bot_id..'zainre'..msg.chat_id_..'')
for k,v in pairs(list) do
redis:del('SNIPER:'..bot_id..'zainre'..msg.chat_id_..'', text)
redis:del('SNIPER:'..bot_id..'zain'..v..''..msg.chat_id_)
redis:del('SNIPER:'..bot_id..':sticker:'..v..msg.chat_id_)
redis:del('SNIPER:'..bot_id..':voice:'..v..msg.chat_id_)
redis:del('SNIPER:'..bot_id..':video:'..v..msg.chat_id_)
redis:del('SNIPER:'..bot_id..':gif:'..v..msg.chat_id_)
redis:del('SNIPER:'..bot_id..':file:'..v..msg.chat_id_)
redis:del('SNIPER:'..bot_id..'zain'..v..msg.chat_id_)
end
...
local list = redis:smembers('SNIPER:'..bot_id..'zainresudo')
for k,v in pairs(list) do
redis:del('SNIPER:'..bot_id..'zainresudo', text)
redis:del('SNIPER:'..bot_id..'zain'..v..'')
redis:del('SNIPER:'..bot_id..':sticker:'..v)
redis:del('SNIPER:'..bot_id..':voice:'..v)
redis:del('SNIPER:'..bot_id..':video:'..v)
redis:del('SNIPER:'..bot_id..':gif:'..v)
redis:del('SNIPER:'..bot_id..':file:'..v)
redis:del('SNIPER:'..bot_id..'zain'..v)
end
...
database:set('SNIPER:'..bot_id..'rules'..msg.chat_id_, txt[2])
send(msg.chat_id_, msg.id_, 1, "✔┇تم وضع القوانين للمجموعه", 1, 'md')
end
if text:match("^القوانين$") then
local rules = database:get('SNIPER:'..bot_id..'rules'..msg.chat_id_)
if rules then
...
if not database:get("group_edit:SNIPER"..msg.chat_id_..bot_id) then
...
database:set('SNIPER:'..bot_id..'setphoto'..msg.chat_id_..':'..msg.sender_user_id_,true)
...
if database:get("group_edit:SNIPER"..msg.chat_id_..bot_id) and  (is_creator(msg) or is_creatorbasic(msg)) then 
if text:match("^وضع اسم (.*)$")   then
local txt = {string.match(text, "^(وضع اسم) (.*)$")}
changetitle(msg.chat_id_, txt[2])
send(msg.chat_id_, msg.id_, 1, '✔┇تم تحديث اسم المجموعه الى \n'..txt[2], 1, 'md')
...
if database:sismember('SNIPER:'..bot_id..'pro:groups',msg.chat_id_) then
...
database:sadd('SNIPER:'..bot_id..'pro:groups',msg.chat_id_)
end
end
if (text and text == 'تعطيل وضع المدفوع') and tonumber(msg.sender_user_id_) == tonumber(sudo_add) then
if not database:sismember('SNIPER:'..bot_id..'pro:groups',msg.chat_id_) then
...
database:del("SNIPER:all_if:"..text..bot_id..msg.chat_id_)
...
database:set("SNIPER:name_sudo"..bot_id..msg.chat_id_,ac[1])
...
database:set("SNIPER:name_cre"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
...
database:set("SNIPER:name_vipp"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
-------------------------------
if text:match("^تغير رد العضو فقط (.*)$") and (is_creator(msg) or is_creatorbasic(msg)) then
ac = {string.match(text, "^تغير رد العضو فقط (.*)$")}
database:set("SNIPER:name_nk"..bot_id..msg.chat_id_,ac[1])
send(msg.chat_id_, msg.id_, 1, "تم تغير الرد الى --> "..ac[1], 1, 'md')
end
--------------------------------
if (text and text == "اضف صلاحيه" and (is_creator(msg) or is_creatorbasic(msg))) then 
send(msg.chat_id_, msg.id_, 1, '☑┇ارسال الان اسم الصلاحيه', 1, 'md')
database:set("SNIPER:new:if:"..bot_id..msg.chat_id_..msg.sender_user_id_,"ok1")
return "SNIPER"
end
if text and database:get("SNIPER:new:if:"..bot_id..msg.chat_id_..msg.sender_user_id_) then 
database:del("SNIPER:new:if:"..bot_id..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل نوع الصلاحيه المطلوبه \n🔰┇{ `مدير` ،`ادمن` ،`عضو مميز` ,`لا شيئ `}', 1, 'md')
database:set("SNIPER:new:if:text:"..bot_id..msg.chat_id_..msg.sender_user_id_,text)
database:set("SNIPER:new:if2:"..bot_id..msg.chat_id_..msg.sender_user_id_,"ok1")
return "SNIPER"
end
if (text and database:get("SNIPER:new:if2:"..bot_id..msg.chat_id_..msg.sender_user_id_)) then 
if (text == "ادمن" or text == "مدير" or text == "عضو مميز" or text == "لا شيئ") then 
database:del("SNIPER:new:if2:"..bot_id..msg.chat_id_..msg.sender_user_id_)
tsha_text = database:get("SNIPER:new:if:text:"..bot_id..msg.chat_id_..msg.sender_user_id_)
send(msg.chat_id_, msg.id_, 1, '☑️┇تم حفض الصلاحيه بنجاح \n⚠️┇الاوامر { رفع/تنزيل '..tsha_text..' }', 1, 'md')
database:set("SNIPER:all_if:"..tsha_text..bot_id..msg.chat_id_,text)
database:del("SNIPER:new:if:text:"..bot_id..msg.chat_id_..msg.sender_user_id_)
else 
send(msg.chat_id_, msg.id_, 1, '📥┇الان ارسل نوع الصلاحيه المطلوبه \n🔰┇{ `مدير` ،`ادمن` ،`عضو مميز` ,`لا شيئ `}', 1, 'md')
end
end
...
ts_text = database:get("SNIPER:all_if:"..a[1]..bot_id..msg.chat_id_)
if ts_text then 
function promote_by_reply(extra, result, success)
if ts_text == "ادمن" then 
database:sadd('SNIPER:'..bot_id..'mods:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "عضو مميز" then 
database:sadd('SNIPER:'..bot_id..'vipgp:'..msg.chat_id_, result.sender_user_id_)
elseif ts_text == "مدير" then 
database:sadd('SNIPER:'..bot_id..'owners:'..msg.chat_id_, result.sender_user_id_)
end
database:set("SNIPER:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_,a[1])
function zain33332(extra,result2,success)
info = '💁🏻‍♂️┇ العضو ✓['..result2.first_name_..'](t.me/'..(result2.username_ or 'TEAMSNIPER')..')\n☑┇تم رفعه '..a[1]
send(msg.chat_id_, msg.id_, 1,info, 1, 'md')
end
...
ts_text = database:get("SNIPER:all_if:"..apmd[1]..bot_id..msg.chat_id_)
if ts_text then 
function promote_by_username(extra, result, success)
if result.id_ then
if ts_text == "ادمن" then 
...
database:set("SNIPER:name_user:"..bot_id..msg.chat_id_..result.id_,apmd[1])
texts = '💁🏻‍♂️┇ العضو ✓['..result.title_..'](t.me/'..(apmd[2] or 'TEAMSNIPER')..')\n☑┇تم رفعه '..apmd[1]
else
texts = '✖┇خطاء'
end
...
database:sadd('SNIPER:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
elseif ts_text == "مدير" then 
database:sadd('SNIPER:'..bot_id..'owners:'..msg.chat_id_, apmd[2])
end
database:set("SNIPER:name_user:"..bot_id..msg.chat_id_..apmd[2],apmd[1])
...
database:del("SNIPER:name_user:"..bot_id..msg.chat_id_..result.sender_user_id_)
...
if result.id_ then
if ts_text == "ادمن" then 
database:srem('SNIPER:'..bot_id..'mods:'..msg.chat_id_, result.id_)
elseif ts_text == "عضو مميز" then 
database:srem('SNIPER:'..bot_id..'vipgp:'..msg.chat_id_, result.id_)
elseif ts_text == "مدير" then 
database:srem('SNIPER:'..bot_id..'owners:'..msg.chat_id_, result.id_)
end
texts = '💁🏻‍♂️┇ العضو ✓['..result.title_..'](t.me/'..(apmd[2] or 'TEAMSNIPER')..')\n☑┇تم تنزيله من '..apmd[1]
database:del("SNIPER:name_user:"..bot_id..msg.chat_id_..result.id_)
else
texts = '✖┇خطاء'
end
send(msg.chat_id_, msg.id_, 1, texts, 1, 'md')
...
if ts_text then 
if ts_text == "ادمن" then 
database:srem('SNIPER:'..bot_id..'mods:'..msg.chat_id_, apmd[2])
elseif ts_text == "عضو مميز" then 
database:srem('SNIPER:'..bot_id..'vipgp:'..msg.chat_id_, apmd[2])
elseif ts_text == "مدير" then 
database:srem('SNIPER:'..bot_id..'owners:'..msg.chat_id_, apmd[2])
end
tsX000(apmd[2],msg,"☑┇ تم تنزيله من "..apmd[1])
database:del("SNIPER:name_user:"..bot_id..msg.chat_id_..apmd[2])
end
end
end 
...
database:del('SNIPER:'..bot_id..'repsudo:mute'..msg.chat_id_)
end
end
...
database:set('SNIPER:'..bot_id..'repsudo:mute'..msg.chat_id_,true)
end
end
...
database:del('SNIPER:'..bot_id..'repowner:mute'..msg.chat_id_)
end
end
...
database:set('SNIPER:'..bot_id..'repowner:mute'..msg.chat_id_,true)
end
end
...
database:del('SNIPER:'..bot_id..'id:mute'..msg.chat_id_)
end
end
...
database:set("SNIPER"..bot_id..":sudo:gr",msg.chat_id_)
end
...
database:del("SNIPER"..bot_id..":sudo:gr")
end
if (text:match("^ايدي$") or text:match("^id$") ) and msg.reply_to_message_id_ == 0 then
if not database:sismember('SNIPER:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_,'ايدي') then
      database:sadd('SNIPER:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_,'ايدي')

local zain_info = nil
function zain333(extra,result,success)
zain_info = '@'..(result.username_ or 'لا يوجد')..''
local function getpro(extra, result, success)
local all_photo_SNIPER = result.total_count_ 
local tahna = (database:get('SNIPER:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_) or 0)
if tonumber((database:get('SNIPER:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_) or 0)) == 0 then
nko = '0'
else
nko = (database:get('SNIPER:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_))
end
local user_msgs = ((database:get('SNIPER:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_) or 0) + tahna)
local izain_text = "غير متفاعل"
if user_msgs then 
...
if not database:get('SNIPER:'..bot_id..'id:mute'..msg.chat_id_) then
if not database:get('SNIPER:'..bot_id..'id:photo'..msg.chat_id_) then
if not database:get("SNIPER:gr:id:text:"..bot_id..msg.chat_id_) then 
local zain_text = {
'صورتك فدشي 😘😔❤️  ',
"صارلك شكد مخليه 😐",
"وفالله 😔💘",
...
local SNIPER_new_text = database:get("SNIPER:gr:id:text:"..bot_id..msg.chat_id_)
local SNIPER_new_text = SNIPER_new_text:gsub('#username',(zain_info or 'لا يوجد'))
local SNIPER_new_text = SNIPER_new_text:gsub('#id',(msg.sender_user_id_ or 'لا يوجد'))
local SNIPER_new_text = SNIPER_new_text:gsub('#edit',(edit or 'لا يوجد'))
local SNIPER_new_text = SNIPER_new_text:gsub('#msgs',(user_msgs or 'لا يوجد'))
local SNIPER_new_text = SNIPER_new_text:gsub('#stast',(t or 'لا يوجد'))
local SNIPER_new_text = SNIPER_new_text:gsub('#auto',(izain_text or 'لا يوجد'))
local SNIPER_new_text = SNIPER_new_text:gsub('#photos',(all_photo_SNIPER or 'لا يوجد'))
local SNIPER_new_text = SNIPER_new_text:gsub('#game',(nko or 'لا يوجد'))
sendPhoto(msg.chat_id_, msg.id_, 0, 1, nil, result.photos_[0].sizes_[1].photo_.persistent_id_,SNIPER_new_text,msg.id_,msg.id_.."")
end
else
if msg.sender_user_id_ == tonumber(sudo_add) then
t = 'المطور الاساسي 👨🏻‍🔧'
...
local SNIPER_new_text = SNIPER_new_text:gsub('#photos',(all_photo_SNIPER or 'لا يوجد'))
local SNIPER_new_text = SNIPER_new_text:gsub('#game',(nko or 'لا يوجد'))
send(msg.chat_id_, msg.id_, 1, SNIPER_new_text, 1, 'html')
end   
end
else
end
...
if text then
if database:sismember('SNIPER:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_,text) then
else
database:del('SNIPER:'..bot_id..'spam:id'..msg.sender_user_id_..':'..msg.chat_id_) 
end
end
...
database:set('SNIPER:'..bot_id..'pin:mute'..msg.chat_id_,true)
end
end
...
database:set('SNIPER:'..bot_id..'pinnedmsg'..msg.chat_id_,msg.reply_to_message_id_)
...
database:set('SNIPER:'..bot_id..'viewget'..msg.sender_user_id_,true)
...
redis:del('SNIPER:'..bot_id..'help', text)
redis:del('SNIPER:'..bot_id..'h1', text)
redis:del('SNIPER:'..bot_id..'h2', text)
redis:del('SNIPER:'..bot_id..'h3', text)
redis:del('SNIPER:'..bot_id..'h4', text)
redis:del('SNIPER:'..bot_id..'h5', text)
redis:del('SNIPER:'..bot_id..'h6', text)
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('SNIPER:'..bot_id..'hhh'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local zain2 = redis:get('SNIPER:'..bot_id..'hhh'..msg.sender_user_id_..'')
if zain2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('SNIPER:'..bot_id..'hhh'..msg.sender_user_id_..'', 'no')
redis:set('SNIPER:'..bot_id..'help', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
---------------
if text == 'سمايلات' and database:get('SNIPER:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('SNIPER:'..bot_id..'l:ids'..msg.chat_id_)
katu = {'🍏','🍎','🍐','🍊','🍋','🍌','🍉','🍇','🍓','🍈','🍒','🍑','🍍','🥥','🥝','🍅','🍆','🥑','🥦','🥒','🌶','🌽','🥕','🥔','??','🥐','🍞','🥖','🥨','🧀','🥚','🍳','🥞','🥓','🥩','🍗','🍖','🌭','🍔','🍟','🍕','🥪','🥙','🍼','☕️','🍵','🥤','🍶','🍺','🍻','🏀','⚽️','🏈','⚾️','🎾','🏐','🏉','🎱','🏓','🏸','🥅','🎰','🎮','🎳','🎯','🎲','🎻','🎸','🎺','🥁','🎹','🎼','🎧','🎤','🎬','🎨','🎭','🎪','🎟','🎫','🎗','🏵','🎖','🏆','🥌','🛷','🚕','🚗','🚙','🚌','🚎','🏎','🚓','🚑','🚚','🚛','🚜','🇮🇶','⚔','🛡','🔮','🌡','💣','📌','📍','📓','📗','📂','📅','📪','📫','📬','📭','⏰','📺','🎚','☎️','📡'}
name = katu[math.random(#katu)]
database:set('SNIPER:'..bot_id..'klmos'..msg.chat_id_,name)
...
if text == ''..(database:get('SNIPER:'..bot_id..'klmos'..msg.chat_id_) or 'لفاتع')..'' and not database:get('SNIPER:'..bot_id..'l:ids'..msg.chat_id_) then
if not database:get('SNIPER:'..bot_id..'l:ids'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n👨🏽‍💻| لعب مره اخره ارسل سمايلات ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('SNIPER:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)  
database:incrby('SNIPER:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    

end
database:set('SNIPER:'..bot_id..'l:ids'..msg.chat_id_,true)
end 
if text == 'الاسرع' and database:get('SNIPER:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('SNIPER:'..bot_id..'l:id'..msg.chat_id_)
...
name = katu[math.random(#katu)]
database:set('SNIPER:'..bot_id..'klmo'..msg.chat_id_,name)
...
if text == ''..(database:get('SNIPER:'..bot_id..'klmo'..msg.chat_id_) or 'لفاتع')..'' and not database:get('SNIPER:'..bot_id..'l:id'..msg.chat_id_) then
if not database:get('SNIPER:'..bot_id..'l:id'..msg.chat_id_) then 
...
database:set('SNIPER:'..bot_id..'l:id'..msg.chat_id_,true)
end 
if text == 'حزوره' and database:get('SNIPER:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('SNIPER:'..bot_id..'l:id1'..msg.chat_id_)
...
name = katu[math.random(#katu)]
database:set('SNIPER:'..bot_id..'klmoa'..msg.chat_id_,name)
...
if text == ''..(database:get('SNIPER:'..bot_id..'klmoa'..msg.chat_id_) or 'لفاتع')..'' and not database:get('SNIPER:'..bot_id..'l:id1'..msg.chat_id_) then
if not database:get('SNIPER:'..bot_id..'l:id1'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n👨🏽‍💻| لعب مره اخره ارسل حزوره ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('SNIPER:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)
database:incrby('SNIPER:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
database:set('SNIPER:'..bot_id..'l:id1'..msg.chat_id_,true)
end 
if text == 'المعاني' and database:get('SNIPER:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('SNIPER:'..bot_id..'l:id2'..msg.chat_id_)
...
name = katu[math.random(#katu)]
database:set('SNIPER:'..bot_id..'means'..msg.chat_id_,name)
...
if text == ''..(database:get('SNIPER:'..bot_id..'means'..msg.chat_id_) or 'لفاتع')..'' and not database:get('SNIPER:'..bot_id..'l:id2'..msg.chat_id_) then
if not database:get('SNIPER:'..bot_id..'l:id2'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n👨🏽‍💻| لعب مره اخره ارسل المعاني ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('SNIPER:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)
database:incrby('SNIPER:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
database:set('SNIPER:'..bot_id..'l:id2'..msg.chat_id_,true)
end 
if text == 'العكس' and database:get('SNIPER:'..bot_id..'lock_geam'..msg.chat_id_) then
database:del('SNIPER:'..bot_id..'l:id3'..msg.chat_id_)
katu = {'باي','فهمت','موزين','اسمعك','احبك','موحلو','نضيف','حاره','ناصي','جوه','سريع','ونسه','طويل','سمين','ضعيف','شريف','شجاع','رحت','عدل','نشيط','شبعان','موعطشان','خوش ولد','اني','هادئ'}
name = katu[math.random(#katu)]
database:set('SNIPER:'..bot_id..'aks'..msg.chat_id_,name)
...
if text == ''..(database:get('SNIPER:'..bot_id..'aks'..msg.chat_id_) or 'لفاتع')..'' and not database:get('SNIPER:'..bot_id..'l:id3'..msg.chat_id_) then
if not database:get('SNIPER:'..bot_id..'l:id3'..msg.chat_id_) then 
taha = '*👾| مبروك لقد فزت ,\n👨🏽‍💻| لعب مره اخره ارسل العكس ,*'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('SNIPER:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)
database:incrby('SNIPER:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
end
database:set('SNIPER:'..bot_id..'l:id3'..msg.chat_id_,true)
end 

if database:get('SNIPER:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then  
...
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
return false  end 
local GETNUM = database:get('SNIPER:'..bot_id.."GAMES:NUM"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
database:del('SNIPER:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_)
database:del('SNIPER:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
database:incrby('SNIPER:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 5)  
database:incrby('SNIPER:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 5)    
taha = '*🔖¦ مبروك فزت ويانه وخمنت الرقم الصحيح\n🚸¦ تم اضافة { 5 } من النقاط *\n'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
elseif tonumber(NUM) ~= tonumber(GETNUM) then
database:incrby('SNIPER:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_,1)
if tonumber(database:get('SNIPER:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_)) >= 3 then
database:del('SNIPER:'..bot_id..'SADD:NUM'..msg.chat_id_..msg.sender_user_id_)
database:del('SNIPER:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
...
if database:get('SNIPER:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_) then  
if text:match("^(%d+)$") then
local NUM = text:match("^(%d+)$")
...
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
return false  end 
local GETNUM = database:get('SNIPER:'..bot_id.."GAMES"..msg.chat_id_)
if tonumber(NUM) == tonumber(GETNUM) then
database:del('SNIPER:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
taha = '*📮¦ مبروك فزت وطلعت المحيبس بل ايد رقم { '..NUM..' }\n🎊¦ لقد حصلت على { 3 }من نقاط يمكنك استبدالهن برسائل *'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:incrby('SNIPER:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 3)  
database:incrby('SNIPER:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 3)    
elseif tonumber(NUM) ~= tonumber(GETNUM) then
database:del('SNIPER:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_)   
...
if text == database:get('SNIPER:'..bot_id..':Set_alii:'..msg.chat_id_) then -- // المختلف
database:incrby('SNIPER:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)  
database:incrby('SNIPER:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
database:del('SNIPER:'..bot_id..':Set_alii:'..msg.chat_id_)
taha = '*  🎁  احسنت اجابتك صحيحه   \n* '
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
end

if text == database:get('SNIPER:'..bot_id..':Set_Amthlh:'..msg.chat_id_) then -- // امثله
database:incrby('SNIPER:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_, 1)  
database:incrby('SNIPER:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_, 1)    
database:del('SNIPER:'..bot_id..':Set_Amthlh:'..msg.chat_id_)
taha = '*  🎁  احسنت اجابتك صحيحه   \n* '
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
end

if text == 'المختلف' and database:get('SNIPER:'..bot_id..'lock_geam'..msg.chat_id_) then
katu = {'😸','☠','🐼','🐇','🌑','🌚','⭐️','✨','⛈','🌥','⛄️','👨‍🔬','👨‍💻','👨‍🔧','👩‍🍳','🧚‍♀','🧜‍♂','🧝‍♂','🙍‍♂','🧖‍♂','👬','👨‍👨‍👧','🕒','🕤','⌛️','📅',
};
name = katu[math.random(#katu)]
database:set('SNIPER:'..bot_id..':Set_alii:'..msg.chat_id_,name)
name = string.gsub(name,'😸','😹😹😹😹😹😹😹😹😸😹😹😹😹')
name = string.gsub(name,'☠','💀💀💀💀💀💀💀☠💀💀💀💀💀')
name = string.gsub(name,'🐼','👻👻👻👻👻👻👻🐼👻👻👻👻👻')
name = string.gsub(name,'🐇','🕊🕊🕊🕊🕊🐇🕊🕊🕊🕊')
...
if text == 'امثله' and database:get('SNIPER:'..bot_id..'lock_geam'..msg.chat_id_) then
katu = {
'جوز','ضراطه','الحبل','الحافي','شقره','بيدك','سلايه','النخله','الخيل','حداد','المبلل','يركص','قرد','العنب','العمه','الخبز','بالحصاد','شهر','شكه','يكحله',
};
name = katu[math.random(#katu)]
database:set('SNIPER:'..bot_id..':Set_Amthlh:'..msg.chat_id_,name)
...
if database:get('SNIPER:'..bot_id..'lock_geam'..msg.chat_id_) then   
Num = math.random(1,6)
database:set('SNIPER:'..bot_id.."GAMES"..msg.chat_id_,Num) 
TEST = [[
*➀       ➁     ➂      ➃      ➄     ➅
↓      ↓     ↓      ↓     ↓     ↓
👊 ‹› 👊 ‹› 👊 ‹› 👊 ‹› 👊 ‹› 👊
...
send(msg.chat_id_, msg.id_, 1,TEST, 1, 'md')
database:setex('SNIPER:'..bot_id.."SET:GAME" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 100, true)  
return false  
end
end
...
database:set('SNIPER:'..bot_id.."GAMES:NUM"..msg.chat_id_,Num) 
TEST = '*\n📮¦ اهلا بك عزيزي في لعبة التخمين :\nٴ━━━━━━━━━━\n'..'⚠¦ ملاحظه لديك { 3 } محاولات فقط فكر قبل ارسال تخمينك \n\n'..'🔖¦ سيتم تخمين عدد ما بين ال {1 و 20} اذا تعتقد انك تستطيع الفوز جرب واللعب الان ؟ \n💥*'
send(msg.chat_id_, msg.id_, 1,TEST, 1, 'md')
database:setex('SNIPER:'..bot_id.."GAME:TKMEN" .. msg.chat_id_ .. "" .. msg.sender_user_id_, 100, true)  
return false  
end
end
...
if tonumber((database:get('SNIPER:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_) or 0)) == 0 then
taha = '*💎¦ ليس لديك مجوهرات \n📬¦ للحصول على مجوهرات ارسل الاسرع وابدأ اللعب*\n'
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
else
taha = '*💎¦ عدد مجوهراتك الحاليه  ('..(database:get('SNIPER:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_))..')*'
taha1 = '*💎¦ مجموع مجوهراتك  ('..(database:get('SNIPER:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_))..')*'
...
database:set('SNIPER:'..bot_id..'gamepoint' .. msg.chat_id_, zain[2] or 50)
...
taha = (database:get('SNIPER:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_) * tonumber(database:get('SNIPER:'..bot_id..'gamepoint' .. msg.chat_id_)or 50))
database:incrby('SNIPER:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_,taha)  
database:del('SNIPER:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_)
taha = tonumber((database:get('SNIPER:'..bot_id..'gamepoint' .. msg.chat_id_) or 50))
send(msg.chat_id_, msg.id_, 1,'💎¦ تم بيع جواهرك كل مجوهره تساوي '..taha..' رساله', 'md')
end
end
if text == 'حذف مجوهراتي' then
database:del('SNIPER:'..bot_id..'add:num'..msg.chat_id_..msg.sender_user_id_)  
database:del('SNIPER:'..bot_id..'add:numall'..msg.chat_id_..msg.sender_user_id_)  
send(msg.chat_id_, msg.id_, 1, "تم حذف جميع مجوهراتك", 1, "md") 
end
if text == 'حذف رسائلي' then
database:del('SNIPER:'..bot_id..'nummsg'..msg.chat_id_..msg.sender_user_id_)
database:del('SNIPER:'..bot_id..'user:msgs'..msg.chat_id_..':'..msg.sender_user_id_)
...
database:set('SNIPER:'..bot_id..'lock_geam'..msg.chat_id_,true)  
end
if text == 'تعطيل الالعاب' and (is_owner(msg) or is_creatorbasic(msg)) then  
taha = '❗️┇ تم تعطيل الالعاب  ✓' 
send(msg.chat_id_, msg.id_, 1,taha, 1, 'md')
database:del('SNIPER:'..bot_id..'lock_geam'..msg.chat_id_) 
end
----------------
if text:match("^الاوامر$") and (is_mod(msg) or is_creatorbasic(msg)) then
local help = redis:get('SNIPER:'..bot_id..'help')
local text =  [[
┇ source ᴏɴᴇ ɪɴ ɪʀᴀǫ 🔱 »
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('SNIPER:'..bot_id..'h11'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local zain2 = redis:get('SNIPER:'..bot_id..'h11'..msg.sender_user_id_..'')
if zain2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('SNIPER:'..bot_id..'h11'..msg.sender_user_id_..'', 'no')
redis:set('SNIPER:'..bot_id..'h1', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
if (text:match("^م1$") or text == "م١") and (is_mod(msg) or is_creatorbasic(msg)) then
local h1 = redis:get('SNIPER:'..bot_id..'h1')
local text =  [[
┇ source ᴏɴᴇ ɪɴ ɪʀᴀǫ 🔱 »
♦️➖▪️➖▪️➖▪️➖▪️➖♦️
┇ اوامر حمايه بالمسح  🔰
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('SNIPER:'..bot_id..'h22'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local zain2 = redis:get('SNIPER:'..bot_id..'h22'..msg.sender_user_id_..'')
if zain2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('SNIPER:'..bot_id..'h22'..msg.sender_user_id_..'', 'no')
redis:set('SNIPER:'..bot_id..'h2', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
if (text:match("^م2$") or text == "م٢") and (is_mod(msg) or is_creatorbasic(msg)) then
local h2 = redis:get('SNIPER:'..bot_id..'h2')
local text =  [[
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('SNIPER:'..bot_id..'h33'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local zain2 = redis:get('SNIPER:'..bot_id..'h33'..msg.sender_user_id_..'')
if zain2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('SNIPER:'..bot_id..'h33'..msg.sender_user_id_..'', 'no')
redis:set('SNIPER:'..bot_id..'h3', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return false end
end
if (text:match("^م3$") or text == "م٣") and (is_mod(msg) or is_creatorbasic(msg)) then
local h3 = redis:get('SNIPER:'..bot_id..'h3')
local text =  [[
┇ source ᴏɴᴇ ɪɴ ɪʀᴀǫ 🔱 »
♦️➖▪️➖▪️➖▪️➖▪️➖♦️
┇ اوامر المدراء  🤵🏻
...
send(msg.chat_id_, msg.id_, 1, '📥┇الان يمكنك ارسال الكليشه  ليتم حفظها', 1, 'html')
redis:set('SNIPER:'..bot_id..'h44'..msg.sender_user_id_..'', 'msg')
return false end
if text:match("^(.*)$") then
local zain2 = redis:get('SNIPER:'..bot_id..'h44'..msg.sender_user_id_..'')
if zain2 == 'msg' then
send(msg.chat_id_, msg.id_, 1, '☑┇تم حفظ الكليشه يمكنك اظهارها بارسال الامر', 1, 'html')
redis:set('SNIPER:'..bot_id..'h44'..msg.sender_user_id_..'', 'no')
redis:set('SNIPER:'..bot_id..'h4', text)
send(msg.chat_id_, msg.id_, 1, text , 1, 'html')
return
false
end
...
local h4 = redis:get('SNIPER:'..bot_id..'h4')
local text =  [[
┇ source ᴏɴᴇ ɪɴ ɪʀᴀǫ 🔱 »
♦️➖▪️➖▪️➖▪️➖▪️➖♦️
┇ اوامر  المطورين  👨‍💻
...
📜┇[قناه شروحات سورس](http://t.me/TEAMSNIPER)
📥┇[قناه ملفات السورس](t.me/TEAMSNIPER)


🔎┇[طريقه التنصيب](https://t.me/SNIEBR/1)
...
z_SNIPER = TSCheckMsg(msg)
if z_SNIPER and z_SNIPER == "stop" then 
if msg.content_.ID == "MessageChatAddMembers" then
if msg.content_.members_[0].type_.ID == 'UserTypeBot' then
if database:get("lock_bot:SNIPER"..msg.chat_id_..bot_id) then
changeChatMemberStatus(msg.chat_id_, msg.content_.members_[0].id_, "Kicked")
...
if database:get("SNIPER:edit:text:su:new2:"..bot_id..data.message_.chat_id_..data.message_.content_.text_) then
local SNIPER_edit_text = database:get("SNIPER:edit:text:su:new2:"..bot_id..data.message_.chat_id_..data.message_.content_.text_)
if SNIPER_edit_text then
data.message_.content_.text_ = SNIPER_edit_text
end
end
end
...
if database:get('SNIPER:'..bot_id.."bc:in:pv" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) then 
if text and text:match("^الغاء$") or text and text:match("^الغاء ✖$") then   
send(msg.chat_id_, msg.id_, 1, "*📬¦ تم الغاء الاذاعه للمشتركين *\n✓", 1, "md") 
database:del('SNIPER:'..bot_id.."bc:in:pv" .. msg.chat_id_ .. ":" .. msg.sender_user_id_) 
else 
local gps = database:scard('SNIPER:'..bot_id.."userss") or 0 
if msg.content_.text_ then
whatbc = 'الرساله'
local list = database:smembers('SNIPER:'..bot_id..'userss') 
for k,v in pairs(list) do 
...
if is_sudo(msg) then 
if database:get("SNIPER:set_if_bc_new:"..bot_id..msg.sender_user_id_) then 
database:del("SNIPER:set_if_bc_new:"..bot_id..msg.sender_user_id_)
local pro = database:scard('SNIPER:'..bot_id..'pro:groups') or 0
if text then 
local gpss = database:smembers( 'SNIPER:'..bot_id.."groups") or 0
for i=1, #gpss do
if not database:sismember('SNIPER:'..bot_id..'pro:groups', gpss[i]) then
send(gpss[i], 0, 1, text, 1, 'html')		
end					
end
end
if (data.message_.content_.sticker_) then 
gpss = database:smembers( 'SNIPER:'..bot_id.."groups") or 0
for i=1, #gpss do
if not database:sismember('SNIPER:'..bot_id..'pro:groups', gpss[i]) then
...
local zain1 = redis:get('SNIPER:'..bot_id..'zain1'..msg.sender_user_id_..''..msg.chat_id_..'')
if zain1 == 're' then
local zain2 = redis:get('SNIPER:'..bot_id..'msg'..msg.sender_user_id_..''..msg.chat_id_..'')
if text then 
redis:set('SNIPER:'..bot_id..'zain'..zain2..''..msg.chat_id_..'', text)
elseif (data.message_.content_.sticker_) then 
redis:set('SNIPER:'..bot_id..':sticker:'..zain2..''..msg.chat_id_..'', data.message_.content_.sticker_.sticker_.persistent_id_)
elseif (data.message_.content_.voice_) then 
redis:set('SNIPER:'..bot_id..':voice:'..zain2..''..msg.chat_id_..'', data.message_.content_.voice_.voice_.persistent_id_)
elseif (data.message_.content_.video_) then 
redis:set('SNIPER:'..bot_id..':video:'..zain2..''..msg.chat_id_..'', data.message_.content_.video_.video_.persistent_id_)
elseif (data.message_.content_.animation_) then 
redis:set('SNIPER:'..bot_id..':gif:'..zain2..''..msg.chat_id_..'', data.message_.content_.animation_.animation_.persistent_id_)
elseif (data.message_.content_.document_) then
redis:set('SNIPER:'..bot_id..':file:'..zain2..''..msg.chat_id_..'', data.message_.content_.document_.document_.persistent_id_)
else
end -- end if text 
redis:sadd('SNIPER:'..bot_id..'zainre'..msg.chat_id_..'', zain2)
send(msg.chat_id_, msg.id_, 1, "📊┇ تم حفظ الرد  ✓", 1, 'md')
redis:set('SNIPER:'..bot_id..'zain1'..msg.sender_user_id_..''..msg.chat_id_..'', 'no')
end
local zain1 = redis:get('SNIPER:'..bot_id..'zain1'..msg.sender_user_id_..'')
if zain1 == 're' then
local zain2 = redis:get('SNIPER:'..bot_id..'msg'..msg.sender_user_id_..'')
if text then 
redis:set('SNIPER:'..bot_id..'zain'..zain2..'', text)
elseif (msg.content_.sticker_) then 
redis:set('SNIPER:'..bot_id..':sticker:'..zain2, msg.content_.sticker_.sticker_.persistent_id_)
elseif (msg.content_.voice_) then 
redis:set('SNIPER:'..bot_id..':voice:'..zain2, msg.content_.voice_.voice_.persistent_id_)
elseif (msg.content_.video_) then 
redis:set('SNIPER:'..bot_id..':video:'..zain2, msg.content_.video_.video_.persistent_id_)
elseif (msg..content_.animation_) then 
redis:set('SNIPER:'..bot_id..':gif:'..zain2, data.message_.content_.animation_.animation_.persistent_id_)
elseif (msg.content_.document_) then
redis:set('SNIPER:'..bot_id..':file:'..zain2, msg.content_.document_.document_.persistent_id_)
end
redis:sadd('SNIPER:'..bot_id..'zainresudo', zain2)
send(msg.chat_id_, msg.id_, 1, "📊┇ تم حفظ الرد  ✓", 1, 'md')
redis:set('SNIPER:'..bot_id..'zain1'..msg.sender_user_id_..'', 'no')
end
--         »»                 Run SNIPER                         ««              --
...
local svuser = 'SNIPER:'..bot_id..'user:'..result.id_
if username then
...
if database:get('SNIPER:'..bot_id..'welcome:'..msg.chat_id_) then
text = database:get('SNIPER:'..bot_id..'welcome:'..msg.chat_id_)
else
text = '👋🏻┇اهلا بك يا ؛ {fr}\n🙋🏼‍♂️┇نورت الكروب ؛ @{us}'
end
...
if database:get('SNIPER:'..bot_id..'pinnedmsg'..msg.chat_id_) and database:get("lock_pin:SNIPER"..msg.chat_id_..bot_id) then
local pin_id = database:get('SNIPER:'..bot_id..'pinnedmsg'..msg.chat_id_)
...
local photo_zain = download_to_file('https://api.telegram.org/file/bot'..token..'/'..zain6.result.file_path, 'files_SNIPER/'..msg.content_.document_.file_name_)
...
local msgg = result 
database:incr('SNIPER:'..bot_id..'user:editmsg'..msgg.chat_id_..':'..msgg.sender_user_id_)
...
if database:get("lock_edit:SNIPER"..msg.chat_id_..bot_id) and not result.content_.text_ and not is_creator(msgg) then
...
send(msg.chat_id_,0, 1, "[⚠┇#تحذير] \n🔖┇قام شخصآ ما في المجموعه بالتعديل على الميديا يرجى الانتباه \n📫┇الشخص الي قام بالتعديل \n ["..data.first_name_.."](T.ME/TEAMSNIPER)" , 1, 'md') 
end
end
...
if text:match("[Tt][Ee][Ll][Ee][Gg][Rr][Aa][Mm].[Mm][Ee]") or text:match("[Tt].[Mm][Ee]") or text:match("[Tt][Ll][Gg][Rr][Mm].[Mm][Ee]") or text:match("[Tt][Ee][Ll][Ee][Ss][Cc][Oo].[Pp][Ee]") then
if database:get("lock_link.note:SNIPER"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) 
end
end
...
if text:match("@") then
if database:get("lock_username.note:SNIPER"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) 
end 
end 
...
if text:match("#") then
if database:get("lock_tag.note:SNIPER"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) 
end 
end 
...
if text:match("/")  then
if database:get("lock_sarha.note:SNIPER"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) 
end 
end
...
check_filter_words(result, text)
if text:match("[\216-\219][\128-\191]") then
if database:get("lock_ar.note:SNIPER"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end  end
if not is_vip(msgg) then
check_filter_words(result, text)
...
check_filter_words(result, text)
if text:match("[ASDFGHJKLQWERTYUIOPZXCVBNMasdfghjklqwertyuiopzxcvbnm]") then
if database:get("lock_en.note:SNIPER"..msg.chat_id_..bot_id) then
local msgs = {[0] = data.message_id_}
delete_msg(msg.chat_id_,msgs) end end end 
 end 
 end
...
database:srem('SNIPER:'..bot_id..'pro:groups',v) 
database:srem( 'SNIPER:'..bot_id.."groups",v) 
end
if data and data.type_ and data.type_.channel_ and data.type_.channel_.status_ and data.type_.channel_.status_.ID == "ChatMemberStatusLeft" then
database:srem("thsake:gog"..bot_id,v) 
database:srem('SNIPER:'..bot_id..'pro:groups',v) 
...
--[[
___ _ _ ___________ ___________ 
/ ___ | \ | | _ _ | ___ \ ___ | ___ \
\ `-. | \ | | | | | | _ / / | __ | | _ / /
 `-. \. `| | | | __ / | __ || /
/ \ __ / / | \ | _ | | _ | | | | ___ | | \ \
